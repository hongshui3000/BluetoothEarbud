...
/*! \brief Enter 'connecting remote' state
    ...
    pending operations.  Make sure AV streaming is suspended.    
*/    
...
#ifdef INCLUDE_AV
    /* Tell AV task we should pause AV streaming */
...
#ifdef INCLUDE_AV
    /* Tell AV task we can resume AV streaming */
...
static void appHfpEnterConnected(void)
{
    ...
    ConnectionSmUpdateMruDevice(&appGetHfp()->ag_bd_addr);
...
static void appHfpHandleHfpSlcDisconnectIndication(const HFP_SLC_DISCONNECT_IND_T *ind)
{
    ...
    switch (appHfpGetState())
    {
        ...
        case HFP_STATE_CONNECTED_ACTIVE:
        {
            ...
            {
                /* Check that there are no HFP_INTERNAL_HFP_DISCONNECT_REQ message to cancel before attempting to reconnect */
                 if (MessageCancelAll(appGetHfpTask(), HFP_INTERNAL_HFP_DISCONNECT_REQ) == 0)
                 {
                     MAKE_HFP_MESSAGE(HFP_INTERNAL_HFP_CONNECT_REQ);
        
                    /* Send message to HFP task */
                    message->addr = appGetHfp()->ag_bd_addr;
                    message->profile = appGetHfp()->profile;
                    message->flags = appGetHfp()->flags | HFP_DISCONNECT_NO_UI;
                    MessageSendConditionally(appGetHfpTask(), HFP_INTERNAL_HFP_CONNECT_REQ, message,
                                             appConManagerCreateAcl(&appGetHfp()->ag_bd_addr));
                }
    ...
    switch (appHfpGetState())
    {
        ...
        case HFP_STATE_DISCONNECTING:
        {
            ...
            if (cfm->status == hfp_audio_connect_success)
            {
                ...
                /* Setup Kymera audio chain */
                appKymeraScoStart(cfm->audio_sink, cfm->codec, cfm->wesco);
