...
static void createSources(audio_input_context_t *ctx, uint16 sample_size)
{
    ctx->left_source = StreamAudioSource(AUDIO_HARDWARE_I2S, AUDIO_INSTANCE_1, AUDIO_CHANNEL_SLOT_0);
    CsrI2SConfigureSource(ctx->left_source, ctx->sample_rate, sample_size);

    ctx->right_source = StreamAudioSource(AUDIO_HARDWARE_I2S, AUDIO_INSTANCE_1, AUDIO_CHANNEL_SLOT_1);
...
static void connectSourcesToChain(audio_input_context_t *ctx)
{
    StreamConnect(ctx->left_source, ChainGetInput(ctx->chain, audio_input_left));

    if(isStereo(ctx))
    {
        StreamConnect(ctx->right_source, ChainGetInput(ctx->chain, audio_input_right));
    }
}
...
static void connectChainToMixer(audio_input_context_t *ctx)
{
    ...
    connect_data.left_src = ChainGetOutput(ctx->chain, local_audio_output_left);
    connect_data.right_src = isStereo(ctx) ? ChainGetOutput(ctx->chain, local_audio_output_right) : NULL;
