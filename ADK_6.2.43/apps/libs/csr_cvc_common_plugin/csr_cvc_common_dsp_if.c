...
void csrCvcCommonDspLoadDsp(CvcPluginTaskdata *task)
{
    ...
    csrCvcSetAppIndex(index);
    
    /* Load the core kap file, if any */
    kap_file = "cvc_core/cvc_core.kap";
    index = FileFind(FILE_ROOT, kap_file, (uint16) strlen(kap_file));
    if (index == FILE_NONE)
    {
    /*  No core kap; fall back to main file */
        index = csrCvcGetAppIndex();
    }
...
/******************************************************************************/
void csrCvcCommonDspConnectScoSource(Source sco_source, CvcPluginTaskdata * task)
{
...
/******************************************************************************/
void csrCvcCommonDspConnectScoSink(Sink sco_sink, bool two_mic)
{
...
/******************************************************************************/
void csrCvcCommonDspConnectMonoSpeaker(audio_output_t output)
{
...
static void sendModeMessage(CVC_MODES_T dsp_mode, CVC_CALL_STATES_T dsp_call_state)
{
    ...
    uint16 mic_flags = (uint16)((mic_a->is_digital << 1) | mic_b->is_digital);
...
void csrCvcCommonDspConfigureMute(bool mute_microphone, bool mute_speaker)
{
    ...
    if(speaker_mute_changed)
        csrCvcCommonDspConfigureVolume(CsrCvcGetVolume(), mute_speaker);
...
/******************************************************************************/
void csrCvcCommonDspConfigureVolume(int16 volume, bool muted)
{
...
void csrCvcCommonDspConnectTone(AUDIO_PLUGIN_PLAY_TONE_MSG_T * tone_message)
{
    ...
    KalimbaSendMessage(MESSAGE_SET_TONE_RATE_MESSAGE_ID, 8000, MONO_PROMPT, 0, 0);
...
/******************************************************************************/
void csrCvcCommonDspTestReset(void)
{
}

/******************************************************************************/
bool csrCvcCommonDspHandleMixerMessage(Task task, MessageId id)
{
    UNUSED(task);
    UNUSED(id);
    return FALSE;
...
/******************************************************************************/
void csrCvcCommonDspHandleDisconnectEvent(Task task)
{
    SetAudioBusy(task);
}
