...
class DebugInfoAdapter(di.DebugInfoInterface):
    ...
    def get_pm_encoding(self, address):
        ...
        Return value is one of {0 (unknown), 1 (Maxim), 2 (Minim)}.
        @param[in] self Pointer to the current object
        @param[in] address
        """
        try:
            return self._call_debuginfo_pm_addr(address, "get_pm_encoding")
    ...
    def update_patches(self, patches):
        ...
        count = 0
        for _, patch in patches.iteritems():
    ...
    def update_bundles(self, bundles):
        ...
        @param[in] bundles A dictionary of bundles
        """
        for _, debug_info in bundles.iteritems():
    ...
    def _search_by_name(self, search_dict_name, function_name, name, elf_id=None):
        ...
        if elf_id is None:
            ...
            for cur_elf_id, debug_info in self.debug_infos.iteritems():
    ...
    def _call_debuginfo_function(self, elf_id, function_name, *argv, **kwargs):
        ...
        """
        # Access the debuginfo in the standard way
        try:
            debug_info = self.debug_infos[elf_id]
        except KeyError:
            raise ct.BundleMissingError()
