    ...
    def __init__(self, cap_entry, helper, cap_elf_id):
        ...
        try:
            ...
            if self.id >= 0x4000 and self.id <= 0x7FFF:
                ...
                self.name = "(Costumer_Capability:" + cu.hex(self.id) + ")"
...
Please check the available analyses and make sure that the Stream is ahead of OPMGR.""")
    ...
    def get_extra_op_data_type(
        ...
        func_address = func_address.replace("0x", "")
        # get the first match of the address
        address_pos = mmap_lst.find(func_address + ":")
        ...
        while import_time.time() - start_time < gurard_time:
            ...
            if "rts;" in current_line:
                return None

            # if the line has an address it means that it is a compiled section
            # from the listing file.
            list_address = re.search(r'^([0-9A-Fa-f]*)\:', current_line)
            if list_address is not None:
                # regular expression to search for casting of the extra_op_data
                cast_type = re.search(
                    r'\=[\ ]*\([\ ]*([A-Za-z0-9\_]*)[\ ]*\*[\ ]*\).*->[\ ]*extra_op_data.*\;',
                    previouse_line
                )
                if cast_type is not None:
                    return cast_type.group(1)
