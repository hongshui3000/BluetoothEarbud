...
def run_plotter(pt_input):
    ...
    axes = plt.gca()
    axes.get_xaxis().set_major_formatter(ticker.FormatStrFormatter(" 0x%08x "))
    plt.legend(loc='lower right')
    plt.title("Buffer Content")
...
def byte_swap(num, len_bytes=None):
    """
    @brief Takes an int/long of arbitrary length, and returns it with bytes reversed.
    If num doesn't contain a whole number of bytes, it will be zero-extended.
    If len_bytes is provided, num will be interpreted as this width (as if
    it were zero-extended to that length).
    Examples:
      byte_swap(0x1234) -> 0x3412
      byte_swap(0x1234, 4) -> 0x34120000
      byte_swap(0xaaa) -> 0xaa0a
    @param[in] num
    @param[in] len_bytes = None
    """
    # It's probably more Pythonic to call the struct module here, but it's
    # complicated to use.
    if len_bytes is None:
        num_hex_digits = hex(num)[2:]  # remove '0x'
        # Round up to whole num of bytes
        len_bytes = int(math.ceil(len(num_hex_digits) / 2.0))
    result = "0x"
    for _ in range(0, len_bytes):
        ls_byte = num & 0xFF  # extract the LS Byte
        result += ('%02x' % ls_byte)  # convert to hex string, append to result
        num >>= 8  # push next higher byte to LS position
    return int(result, 0)
...
def mem_size_to_string(size_in_octets):
    """
    ...
    @param[out] String which contains the memory size in different
        units.
    """
    # first format the size in bytes
    return_str = "{:7,} bytes".format(size_in_octets)
    if size_in_octets / 1024 != 0:
        return_str += " ({:5.1f} KiB)".format(size_in_octets / 1024.0)
    # if the address per words is different than one, format the size in words.
    if Arch.addr_per_word != 1:
        size_in_words = size_in_octets / Arch.addr_per_word
        return_str += " or {:7,} words".format(size_in_words)
...
def signal_handler(*_):  # signal, frame
    """
    Signal handler for ACAT.
    """
...
def parse_args(parameters):  # pylint: disable=too-many-branches,too-many-statements
    ...
    if args:
        raise ct.Usage("ERROR: Unknown option " + str(args))
