...
static char *pmalloc_end = (char *)pmalloc_begin + PMALLOC_RAM_SIZE;
#endif


/**
 * Configuration of pool blocks.
 * Pools allocations consist of a combination of -
 * 1) Hard-coded numbers of blocks known to be required from the
 * P0 and P1 code-base. These are configured in pmalloc_config.h
 * and should be updated as known per module
 * 2) Remaining allocation spread across block-sizes. This makes it 
 * relatively robust against un-tracked allocation requirements
 */
char *pmalloc_begin_ind = (char *)pmalloc_begin;
#ifdef NO_PMALLOC_MEMMAP_SYMBOLS
char *pmalloc_end_ind = (char *)pmalloc_begin + PMALLOC_RAM_SIZE;
#else
char *pmalloc_end_ind = (char *)pmalloc_end;
#endif
...
void pmalloc_configure(pmalloc_pool_config *pool_config, unsigned pool_config_len)
{
    ...
    pmalloc_pools = (pmalloc_pool *) PMALLOC_ALIGN(pmalloc_begin_ind);
...
void init_pmalloc(void)
{
    ...
    /* Get the calculated configuration, 
     * get_pmalloc_config combines the contributions from
     * different modules in listed in pmalloc_config_P0.h
     * or pmalloc_config_P1.h, appropriately */
