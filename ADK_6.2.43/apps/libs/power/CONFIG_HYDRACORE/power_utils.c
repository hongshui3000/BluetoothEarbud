...
static uint16 powerUtilGetSafeCurrentLimit(bool use_ext, uint16 current, uint16 quiescent)
{
    if (use_ext)
    {
        ...
        if (current > INITIAL_FAST_DYNAMIC_CURRENT_IN_MA)
            current -= quiescent;
    }
    else
    {
...
void powerUtilSetChargerCurrents(uint16 current)
{
    ...
    new_safe_current = powerUtilGetSafeCurrentLimit(power->config.vchg.ext_fast,
                                                    current,
...
{
    return (power->vchg >= PowerChargerGetVtermMillivolts(power->config.vchg.default_vfloat));
}
...
bool powerUtilAdcRequest(Task task, vm_adc_source_type adc_source, bool is_vthm, bool drive_ics, uint16 delay)
{
    ...
    {
        flags = ADC_INTERNAL_CURRENT_SOURCE_EN;
    }
    else
    {
        flags = ADC_STANDARD;
...
uint16 powerUtilConditionVchgReading(uint16 vchg_sns_reading)
{
    ...
    if(power->config.vchg.ext_fast)
    {
        ...
        conditioned_vchg_reading += 
           (uint16)(((uint32)power->charger_i_actual_fast*chg_prg_mon*power->calculated_external_resistance_in_milli_ohms)/
...
bool powerUtilIsBatteryBelowTerminationVoltage(uint16 vbat_millivolts)
{
    ...
    return (vbat_millivolts < PowerChargerGetVtermMillivolts(power->vterm));
}

/****************************************************************************
NAME
    powerUtilGetVtermMillivolts
...
RETURNS
    uint16
*/
uint16 powerUtilGetVtermMillivolts(uint8 vterm)
{
    return MIN_VTERM + (vterm * VTERM_STEP);
}
