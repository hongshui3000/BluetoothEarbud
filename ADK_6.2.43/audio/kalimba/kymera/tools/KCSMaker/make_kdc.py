...
class make_kdc:
    ...
    def buildVariableLut(self, dnld, rom, progLinks, modLut, varLinks, varSizes, extraVarSizes, mappings):
        ...
        def getAddrFromAnonymousVariable(baseAddr, pos, dnld, instrSet, off):
            ...
            pmAddrs = dnld.sym.static_pm.keys()
            pmAddrs.sort()
            static_pm = map( lambda p: dnld.sym.static_pm[p], pmAddrs )
            static_pm_minim = []
            ...
            if instrSet == "MAXIM":
                ...
                logging.debug("Minim instruction (anonymous): " + str(map(hex,prefixes)) + " " + str(hex(instr)))
                if ((instr & 0xC000) == 0x4000):
                    ...
                    logging.debug("Subword instruction (anonymous): " + str(map(hex,prefixes)) + " " + str(hex(instr)))
                    ...
                    logging.debug("Add/Sub (Type B) instruction (anonymous): " + str(map(hex,prefixes)) + " " + str(hex(instr)))
        ...
        def getAddrFromAnonymousVariableVarLut(varLut, varLink, lnk, dnld):
            for v in varLut:
                ...
                if v.name == varLink:
                    ...
                    return dnld.sym.static_dm[v.addr + lnk.pos] - lnk.off
            raise Exception("Error, {} not found in varLut".format(varLink))

        def getRegions(regNameStart, regNameEnd, dnld):
            """ Get the value for the regions given in regNameStart and regNameEnd """
            REGION_ADDR_START = filter( lambda x : x[0] == regNameStart, dnld.sym.variables.items() )
            if len(REGION_ADDR_START) != 0:
                REGION_ADDR_END = filter( lambda x : x[0] == regNameEnd, dnld.sym.variables.items() )
                ...
                REGION_ADDR_START = filter( lambda x : x[0] == regNameStart, dnld.sym.constants.items())
                if len(REGION_ADDR_START) != 0:
                    REGION_ADDR_END = filter( lambda x : x[0] == regNameEnd, dnld.sym.constants.items())
        ...
        def filterVar(varname, ignore_prefixes, ignore_suffixes, mappings):
            ...
            for prefix in ignore_prefixes:
                if varname.startswith(prefix):
                    logging.debug("ignoring variable {v} ({o})".format(v=varname, o=origvarname))
                    return False
        ...
        varItems = filter( varFilt, dnld.sym.variables.items() )
        ...
        file_id_addr = filter(checksumFilt, varItems)
        ...
        if self.chip.does_dm_octet_addressing():
            for i in xrange(len(varItems)):
                if varItems[i][0] == "__devtools_image_checksum":
                    param1 = varItems[i][1][1]
                    param2 = varItems[i][1][2]
                    varItems[i] = ("__devtools_image_checksum", (4L, param1, param2))

        file_id_addr = file_id_addr[0][1][1]
        self.file_id = dnld.sym.static_dm[file_id_addr]

        const16Items = filter( const16Filt, varItems )
        constItems = filter( constFilt, varItems )
        DMConst16Items = filter ( DMConst16Filt, varItems )
        DMConst24Items = filter ( DMConst24Filt, varItems )
        notConstItems =  filter( notConstItems, varItems )

        # get the list of variables and remove the ones we don't care about
        varLut = map( lambda x : Symb(x, "NOT_CONST"), notConstItems )
        const16Lut = map( lambda x : Symb(x, "CONST_16"), const16Items )
        constLut = map( lambda x : Symb(x, "CONST"), constItems )
        DMConst16Lut = map( lambda x : Symb(x, "DMCONST16"), DMConst16Items )
        DMConst24Lut = map( lambda x : Symb(x, "DMCONST24"), DMConst24Items )
        ...
        for prg in progLinks:
            ...
            for lnk in filter(lambda x: x.type == x.TYPE_SE, progLinks[prg]):
        ...
        for varLink in varLinks:
            ...
            if not ignore_varLinks_libs in folders:
                for lnk in filter(lambda x: x.type == x.TYPE_SE, varLinks[varLink]):
                    # An anonymous variable will have the object file name prefixed to its name separated by a '%' char
                    if lnk.name.find("%") != -1:
                        # Non-global variables start with L_ or A_ in gcc compilers, but names might be encrypted,
                        # So check against the mapping table first
        ...
        dic_varLut = dict( map( lambda v: (v.name, v), varLut ) )
    ...
    def _get_label(self, dnld, kld_match_string, klink_match_string):
        ...
        if dnld.__class__ == kalspi.KalSpi:
            ...
            if self.chip.uses_kld():
                val  = filter( lambda l: l[0].endswith(kld_match_string),  dnld.get_all_symbols() )
            else:
                val  = filter( lambda l: l[0].endswith(klink_match_string),  dnld.get_all_symbols() )
            if len(val) != 1:
    ...
    def fixupCodeOffsets(self, progLinks, modLut, dnld):
        ...
        for prog in progLinks:
            ...
            if instrSet == "MAXIM":
                ...
                logging.debug(map(hex,static_pm_minim))
                # Process Minim links
    ...
    def processVariableLinks(self, varLinks, varLut, progLut, dnld, rom, modLut, kdc_idx):
        ...
        for (name, v) in varLut.items():
            ...
            if self.chip.does_dm_octet_addressing():
                ...
                static_32 = map(lambda a: dnld.sym.static_dm.get(a, 0), range(v.addr & ~0x3, v.addr + v.size + unalignment, 4))
                ...
                static = map(lambda a: dnld.sym.static_dm.get(a, 0), xrange(v.addr, v.addr+v.size))

            links  = varLinks.get(v.name, [])
            logging.debug("static: " + str(map(hex,static)))
            ...
            for lnk in links:
                ...
                if varLut.has_key(lnk.name):
                    ...
                    # Variable in DM has a reference in CONST, it's a descriptor and it's a random access KCS, don't patch it
                        ...
                        logging.debug("base_id and var_id patched, new static: " +  str(map(hex,static)))
                ...
                if progLut.has_key(proglut_name):
            ...
            # lose the lot
            lastZero = None
            for i in xrange( len(static)-1, -1, -1):
                if static[i] != 0:
                    break
                lastZero = i
            ...
            for lnk in filter(lambda x: x.type == x.TYPE_SE, progLinks[prg]):
                ...
                if varLut.has_key(lnk.name):
                    ...
                    if modLut[prg][1] == "MAXIM":
                        ...
                        logging.debug("Minim instruction: " + str(map(hex,prefixes)) + " " + str(hex(instr)))
                        ...
                        if ((instr & 0xC000) == 0x8000):
                            ...
                            logging.debug("Add/Sub (Type B) instruction: " + str(map(hex,prefixes)) + " " + str(hex(instr)))
                            # This is a prefixed Addition/Subtraction instruction
                            # Two prefixes expected
                            if len(prefixes) != 2:
                                raise Exception("Unsupported instruction type. Wrong number of prefixes: %d" % len(prefixes))
                           ...
                           logging.debug("Insert32 instruction: " + str(map(hex,prefixes)) + " " + str(hex(instr)))
                        ...
                        # Save for later
                        addrSaved = addr
                        logging.debug("Minim instruction patched: " + str(map(hex,patchedInstr)))
    ...
    def getMaximAddresses(self, modLut, dnld):
        ...
        startAddr.sort(lambda x,y: cmp(x[1], y[1]))
        ...
        if (processingMaxim == True):
            ...
            static_pm = dnld.sym.static_pm.keys()
            static_pm.sort()
    ...
    def addVarSections(self, kdc, variables):
        ...
        for v in variables:
            ...
            if (v.var.type == "CONST_16"):
                ...
                if self.chip.does_dm_octet_addressing():
                    for i in xrange(len(v.static_data)):
                        if i & 1:
    ...
    def addCONST16(self, kdc, variables):
        ...
        if self.chip.does_dm_octet_addressing():
            for v in variables:
                ...
                for j in xrange(i, v.var.size - 1):
                    if i & 1:
                        kdc.append(0)
    ...
    def addCONST(self, kdc, variables):
        ...
        if self.chip.does_dm_octet_addressing():
            for v in variables:
                ...
                for j in xrange(((len(v.static_data)+3)/4)*4, v.var.size):
                    if j & 1:
                        kdc.append(0x0)
                sectSize = sectSize + (v.var.size / 2)
        else:
            for v in variables:
                for i in xrange(len(v.static_data)):
                        ...
                        kdc.append( v.static_data[i] & 0xFFFF )
                # Append zeros until variable size is reached
                for j in xrange(len(v.static_data), v.var.size):
                    kdc.append(0)
                    kdc.append(0)
                sectSize = sectSize + v.var.size * 2
    ...
    def parseELF(self, ker, dnld, len_static_pm):
        ...
        pm_section_keys = filter(lambda x: (section_hdrs[x].type == ker.KerElfSectionHeaderInfo.ELF_SECTION_TYPE_MAXIM) or (section_hdrs[x].type == ker.KerElfSectionHeaderInfo.ELF_SECTION_TYPE_MINIM), section_hdrs)
        ...
        base_dm2 = self.getAddr(dnld, DM2_REGION_BASE)
        limit_dm2 = self.getAddr(dnld, DM2_REGION_LIMIT)
        dm_section_keys = filter(lambda x: section_hdrs[x].type == ker.KerElfSectionHeaderInfo.ELF_SECTION_TYPE_DATA, section_hdrs)
    ...
    def generateKDC(self, kdc_idx, rom):
        ...
        # Import any exported relocation data
        #if os.path.isfile(relocFile) and os.path.dirname(relocFile).find("lib_release") != -1:
        mappings = {}
        mappings_to_export = {}
        func_import = {}
        extraVarSizes = {}
        for lib_name in self.oFiles:
            ...
            if os.path.isfile(reloc_file):
                ...
                if os.path.dirname(reloc_file).find("lib_release") == -1:
                    raise Exception("Error. reloc file found under an unexpected path (Not under lib_release): {}".format(reloc_file))
        ...
        for lib_name in func_export:
            with open("{}.reloc".format(os.path.splitext(lib_name)[0]),"wb") as output:
                ...
                extraVarSizes = {k: v for k, v in extraVarSizes.iteritems() if k in publicSymbols}
        ...
        garbage = filter(lambda x: not modLut.has_key(x), progLinks)
        for sym in garbage:
        ...
        garbage = filter(lambda x: not varLut.has_key(x), varLinks)
        for sym in garbage:
            varLinks.pop(sym)
        ...
        DMVars = filter(lambda x : x.var.type == "NOT_CONST" or x.var.type == "DMCONST16" or x.var.type == "DMCONST24", variables)
        logging.debug("DMVars:" + str(DMVars))
        Const16 = filter(lambda x : x.var.type == "CONST_16", variables)
        logging.debug("Const16:" + str(Const16))
        Const = filter(lambda x : x.var.type == "CONST", variables)
