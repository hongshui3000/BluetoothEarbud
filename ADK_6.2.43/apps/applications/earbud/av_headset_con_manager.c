...
static void appConManagerSetLinkSupervisionTimeout(const bdaddr *bd_addr, uint16 timeout)
{
...
{
    uint16 page_timeout = appConfigDefaultPageTimeout();
    ...
    if (appDeviceIsPeer(addr))
    {
        if (appConfigIsLeft())
        {
            page_timeout = appConfigLeftEarbudPageTimeout();
        }
        else
        {
            page_timeout = appConfigRightEarbudPageTimeout();
        }
    }
...
void appConManagerHandleClDmAclOpenedIndication(const CL_DM_ACL_OPENED_IND_T *ind)
{
    ...
    {        
        ...
        /* Set default link supervision timeout (convert milliseconds to slots) if locally inititated (i.e. we're master) */
        if (is_local)
            appConManagerSetLinkSupervisionTimeout(&ind->bd_addr.addr, appConfigDefaultLinkSupervisionTimeout() * 1000UL / 625);
