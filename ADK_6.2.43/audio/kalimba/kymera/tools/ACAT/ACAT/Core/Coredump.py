...
class Coredump(ChipData.ChipData):
    ...
    def __init__(self, coredump_filepath, processor):
        ...
        if isinstance(self.coredump_filepath, list):
            self._read_xcd_file(
                itertools.imap(lambda x: x.rstrip(), self.coredump_filepath)
                .__iter__()
            )
        else:
            with open(self.coredump_filepath) as coredump_file:
                self._read_xcd_file(
                    itertools.imap(lambda x: x.rstrip(), coredump_file)
                    .__iter__()
                )
    ...
    def set_data(self, address, value):
        ...
        @param[in] value
        """
        raise StandardError("Cannot set data for coredump.")
...
def main():
    ...
    print("Coredump path is ", coredump_path)
