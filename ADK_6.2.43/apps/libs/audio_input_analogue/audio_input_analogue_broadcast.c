...
void audioInputAnalogueBroadcastCreate(Task input_task, Task output_task, audio_input_context_t *ctx)
{
    ...
    params.forwarding.pcm.left_source = ChainGetOutput(ctx->chain, local_audio_output_left);
    params.forwarding.pcm.right_source = ChainGetOutput(ctx->chain, local_audio_output_right);
