...
class Buffers(Analysis.Analysis):
    ...
    def analyse_transform_buffers(
            self, analyse_metadata=False, tag_names=None
        ):
        """
        ...
        @param[in] tag_names List of metadata tag names to display. Default
                    value is None (no tag names will be displayed)
        """
        try:
            ...
            for transform in streams_analysis.get_transforms():
                self.analyse_cbuffer(
                    transform.buffer, analyse_metadata, tag_names
                )
    ...
    def analyse_cbuffer(
        ...
        @param[in] tag_names List of metadata tag names to display. Default
                    value is None (no tag names will be displayed)
        """
        buff_var = self.inspect_cbuffer(buffer_p, analyse_metadata, tag_names)
        namestr = str(buff_var.buffer_type) + '-flavoured cbuffer'
        self.formatter.section_start(namestr)
        self.formatter.output(buff_var)
        ...
        if not plt is None:
            ...
            self.plot_content(buff_var, cu.u32_to_frac32)
        self.formatter.section_end()
    ...
    def _get_content(self, cbuffer, start_with="base",
        ...
        @param[out] buffer content as a dictionary or list of values depending
            on the format.
        """
        if isinstance(cbuffer, (int, long)):
    ...
    def plot_content(self, cbuffer, conversion_function=None):
        """
        ...
        if cu.global_options.under_test:
            ...
            pt_output = re.sub(r"#[a-z0-9]{11}", "", pt_output, flags=re.M)
            pt_output = re.sub(r"id=\"[a-z0-9]{11}\"", "id=\"\"", pt_output, flags=re.M)
    ...
    def inspect_cbuffer(
            self, buffer_p, analyse_metadata=False, tag_names=None
        ):
        """
        @brief Takes a pointer to a cbuffer and inspects its contents.
        ...
        @param[in] tag_names List of metadata tag names to display. Default
                    value is None (no tag names will be displayed)
        """
        buffer_var = Cbuffer(self.chipdata.cast(buffer_p, "tCbuffer"))
        ...
        if analyse_metadata:
            buffer_var.metadata_list = \
                self._analyse_buffer_metadata(buffer_var, tag_names)
    ...
    def draw_cbuffer(self, buffer_p, rd_side='right'):
        ...
        if cbuffer.buffer_type == "Remote MMU":
            ...
            diag.append((" "*12 + "0x{0:0>6x}" + " "*12).format(buffer_p))
            return diag
    ...
    #######################################################################
    def _analyse_buffer_metadata(self, buffer_var, tag_names=None):
        """
        ...
        if buffer_var.get_member('metadata').value and buff_metadata_flag:
            ...
            metadata_list_str = self._get_linked_list_contents(
                metadata_list, tag_names
            )

            first_pointer = buffer_var.get_member('metadata').value

            for metadata in self.parse_linked_list(metadata_list, 'next'):
                if metadata.get_member('next').value != first_pointer:
                    metadata_list_str += self._get_linked_list_contents(
                        metadata, tag_names
                    )
                else:
    ...
    def _get_linked_list_contents(self, metadata_list, tag_names=None):
        """
        ...
        if head_value != 0:
            ...
            count = 0
            # Only print tags if any required, otherwise print the number of
            # them
            ...
            for tag in self.parse_linked_list(head, 'next'):
                ...
                if tag_names is not None:
                    ...
                    metadata_list_str += new_tag.tag_to_string(tag_names)
...
class MetadataTag(object):
    ...
    def __init__(self, tag):
        ...
        for member in self.tag.members:
            self.tag_members[member.name.split('.')[-1]] = cu.hex(member.value)
    ...
    def __str__(self):
        ...
        tag_str = ''
        count = 0
        for tag_member in self.tag_members:
            tag_str += self.tag.indent + \
                ' {0:<10}'.format(tag_member) + ':' + \
                '{0:<15}'.format(self.tag_members[tag_member])
            count += 1
            if count % 3 == 0 and count != 0:
                tag_str += '\n'
        return tag_str

    def tag_to_string(self, tag_names):
        '''
        @brief method takes MetadataTag object and list of asociated parameters
        and returns a string displaying these parameters and information stored
        in them.
        @param[in] self pointer to a current object
        @param[in] tag_names - list of tag parameters to be displayed
        '''
        tag_str = ""
        ...
        if not tag_names:
            ...
            for tag_info in tag_names:
                if self.tag.get_member(tag_info) is None:
                    raise TypeError(
                        'Invalid tag name "{}" entered. They can be: {}\n'.
                        format(tag_info, self.tag_members.keys())
                    )
                else:
                    tag_str += self.tag.indent
                    tag_str += '{0:<10}'.format(tag_info + ':')
                    tag_str += '{0:<15}'.format(
                        cu.hex(self.tag.get_member(tag_info).value)
                    )
                    count += 1
                    if count % 3 == 0 and count != 0:
                        tag_str += '\n'
                    elif tag_names[-1] == tag_info:
                        tag_str += '\n'
