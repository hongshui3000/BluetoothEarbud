...
class Endpoint(object):
    ...
    def __init__(self, ep, name, stream, is_state_type_equal_to_name=True):
        ...
        # connected_to is populated by the transforms

        self.ep_to_kick = ep.get_member('ep_to_kick').value
...
class Transform(object):
    ...
    def __init__(self, t, stream):
        ...
        # populate the connected to field for endpoints
        self.sink.connected_to = self.source
        self.source.connected_to = self.sink
...
class Stream(Analysis.Analysis):
    ...
    def convert_transform_id(id_to_convert, force_direction=None):
        ...
        if not force_direction is None:
    ...
    def run_all(self, cbopsanalysis_on=False):
        ...
        self.analyse_transforms(
            True, True, ['index', 'timestamp', 'sp_adjust', 'length']
        )
    ...
    def analyse_transforms(
            self, display_buffer=False, analyse_metadata=False, tag_names=None
        ):
        """
        ...
        for t in self.transforms:
            ...
            if display_buffer:
                buffers_analysis.analyse_cbuffer(
                    t.buffer, analyse_metadata, tag_names
                )
            self.formatter.section_end()
    ...
    def analyse_chains(self):
        ...
        for cluster_name in graph_dict:
            ...
            source_list = []
            sink_list = []
            for ep_id, details in graph_dict[cluster_name][Graphviz.ELEMENTS].iteritems():
        ...
        try:
            if type(self.formatter) is InteractiveFormatter:
                ...
                if not cu.global_options.under_test:
                    self.formatter.output_svg(graph.pipe(format='svg'))
                    self.formatter.output_svg(legend.pipe(format='svg'))
                else:
    ...
    def _read_endpoint(self, ep):
        ...
        # audio endpoint is treated diffenretly to show the timedplayback
        # module.
        if ep_type == "audio":
