...
*/
static void IsValidatedToTrySwap(void)
{
    ...
    case 1:
        {
            ...
            /*
             * The apps SQIF contains an audio image so the audio needs to be
             * shut down then the ImageUpgradeAudio trap called.
             */
            PRINT(("IsValidatedToTrySwap() apps SQIF has audio\n"));
            UpgradeApplyAudioShutDown();
#else
            PRINT(("ImageUpgradeSwapTry() apps SQIF has no audio\n"));
            ImageUpgradeSwapTry();
#endif
        }
        break;
...
bool UpgradeSMHandleValidated(MessageId id, Message message)
{
    ...
    case UPGRADE_VM_AUDIO_DFU_SUCCESS:
        {
            PRINT(("ImageUpgradeSwapTry() in UPGRADE_VM_AUDIO_DFU_SUCCESS\n"));
            ImageUpgradeSwapTry();
        }
        break;
