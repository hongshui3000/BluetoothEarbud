...
#define BATTERY_READ_PERIOD_INITIAL (100)

/*! When the filter is full, read at this rate */
#define BATTERY_READ_PERIOD (D_SEC(2))
...
static enum battery_level_state toState(uint16 voltage)
{
    if (voltage < appConfigBatteryVoltageLow())
        return battery_level_critical;
    else if (voltage < appConfigBatteryVoltageOk())
...
static bool stateUpdateIsRequired(enum battery_level_state last_state,
                                  enum battery_level_state new_state,
                                  uint16 voltage,
                                  uint16 hysteresis)
{
    switch (last_state)
...
static void appBatteryServiceClients(batteryTaskData *battery)
{
    ...
    for (client = battery->client_list; client != NULL; client = client->next)
    {
        ...
        switch (client->form.representation)
        {
            ...
            {
                enum battery_level_state new_state = toState(voltage);
...
/*! Return TRUE if a new voltage is available */
    ...
    switch (result->adc_source)
    {
        case adcsel_pmu_vbat_sns:
        {
            ...
            /* See the logic in appBatteryGetVoltage():
               ...
               so jump over that range when the index wraps */
        ...
        break;
        default:
            DEBUG_LOG("appBatteryAdcResultHandler unexpected source");
        break;
    }
    return FALSE;
}
...
void appBatteryInit(void)
{
    ...
    battery->period = BATTERY_READ_PERIOD;
...
enum battery_level_state appBatteryGetState(void)
{
    uint16 voltage = appBatteryGetVoltage();
...
{
    batteryTaskData *battery = appGetBattery();
    return appBatteryClientAdd(battery, client);
}
