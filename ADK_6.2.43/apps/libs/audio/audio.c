...
static void audioDisconnectTask(Task audio_plugin, AUDIO_SINK_T sink_type)
{
    ...
    /* Check if Voice Assistant is active before disconnecting any other plugin activity*/
    if(AudioGetVoiceAssistantTask() != NULL)
    {
...
/****************************************************************************
NAME
    AudioSetVoiceAssistantTask
DESCRIPTION
    This method is used to set the voice assistant task
RETURNS

*/

void AudioSetVoiceAssistantTask(Task plugin)
{
    if(AUDIO)
    {
        AUDIO->audio_voice_assistant_plugin = plugin;
    }
}

/****************************************************************************
NAME
    AudioGetVoiceAssistantTask
DESCRIPTION
    This method is used to get the voice assistant task if active
RETURNS
    Voice assistant plugin task
*/

Task AudioGetVoiceAssistantTask(void)
{
    Task voice_assistant_task = (Task)NULL;
    if(AUDIO)
    {
        voice_assistant_task = AUDIO->audio_voice_assistant_plugin;
    }
    return voice_assistant_task;
}
...
{
    MessageSendConditionallyOnTask(AudioGetVaPlugin(), AUDIO_PLUGIN_STOP_VOICE_CAPTURE_MSG, NULL, AudioBusyPtr());
    return TRUE;
}
