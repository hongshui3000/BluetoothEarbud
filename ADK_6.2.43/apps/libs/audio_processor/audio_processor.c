...
{
    PRINT(("AudioProcessorAddUseCase(%ld): %s\n", use_case,
...
{
    PRINT(("AudioProcessorRemoveUseCase(%ld): %s\n", use_case,
           get_use_case_enum_string(use_case)));
    if (use_case < audio_ucid_number_of_ucids)
    {
        if (isUseCaseBitSet(use_case))
        {
            ...
            {
                PRINT(("%ld bit set but count already zero!\n", use_case));
            }
