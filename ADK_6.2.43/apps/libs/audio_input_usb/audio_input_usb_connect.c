...
void AudioInputUsbConnectHandler(Task task, Message message, audio_input_context_t* ctx)
{
    ...
    ctx->left_source = ChainGetOutput(ctx->chain, audio_output_left);
    ctx->right_source = ChainGetOutput(ctx->chain, audio_output_right);
    ...
    StreamConnect(ctx->codec_source, ChainGetInput(ctx->chain, audio_input_pcm));
        ...
        unsigned number_of_channels)
{
    ...
    {       
        ...
        OperatorsStandardSetLatencyLimits(usb_audio_rx_op,
                                          ...
                                          TTP_BA_MAX_LATENCY_LIMIT_US - TTP_WIRED_BA_LATENCY_IN_US);
    }
...
void audioInputUsbConnectToMixer(Task task, Message message, audio_input_context_t* ctx)
{
    ...
    connect_data.right_src = ctx->right_source;
