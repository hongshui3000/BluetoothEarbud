...
static kymera_chain_handle_t createDecoderChain(uint32 rate, codec_config_celt *config);
static bool pluginTypeIsBa(A2DP_DECODER_PLUGIN_TYPE_T plugin_type);
static Sink audioInputBroadcastConnectCreateOperators(audio_input_context_t *ctx, uint32 rate, codec_config_celt *config);
...
void audioInputBroadcastConnectHandler(Task task, Message message, audio_input_context_t *ctx)
{
    ...
    if (pluginTypeIsBa(decoder))
    {
        ...
        audio_frame_sink = audioInputBroadcastConnectCreateOperators(ctx, msg->rate, NULL);
...
static void connectChainToMixer(audio_input_context_t *ctx, uint32 sample_rate)
{
    ...
    connect_data.connection_type = CONNECTION_TYPE_MUSIC_BA;
...
*/
static kymera_chain_handle_t createDecoderChain(uint32 rate, codec_config_celt *codec_config)
{
    const chain_config_t* chain_config = audioInputBroadcastGetChainConfig();
    kymera_chain_handle_t chain = PanicNull(ChainCreate(chain_config));

    configureDecoderChain(chain, rate, codec_config);
...
*/
static Sink audioInputBroadcastConnectCreateOperators(audio_input_context_t *ctx, uint32 rate, codec_config_celt *codec_config)
{
    ctx->chain = createDecoderChain(rate, codec_config);
...
/****************************************************************************
DESCRIPTION
    Forece-reset the plugin to disconnected state. For unit test use only.
*/
void audioInputBroadcastReset(void)
{
