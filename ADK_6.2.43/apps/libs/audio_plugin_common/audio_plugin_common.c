...
/****************************************************************************
DESCRIPTION
    Helper to set PIO state
*/
static bool audioPluginSetPio(unsigned bias_config, unsigned pio, bool set)
{
...
/****************************************************************************
DESCRIPTION
    Set mic bias PIO on or off
*/
void AudioPluginSetMicPio(const audio_mic_params params, bool set)
{
    PanicFalse(audioPluginSetPio(params.bias_config, params.pio, set));
}
...
*/
void AudioPluginInitMicPio(const audio_mic_params audio_mic)
{
    /* Failure here most likely indicates "already off" so ignore it */
    (void)audioPluginSetPio(audio_mic.bias_config, audio_mic.pio, FALSE);
}
...
Source AudioPluginMicSetup(audio_channel channel, const audio_mic_params audio_mic, uint32 rate)
{
    ...
    AudioPluginSetMicPio(audio_mic, TRUE);
    AudioPluginMicConnected(channel, &audio_mic);
    return mic_source;
}
...
void AudioPluginMicShutdown(audio_channel channel, const audio_mic_params * params, bool close_mic)
{
    ...
    AudioPluginSetMicPio(*params, FALSE);
    AudioPluginMicDisconnected(channel, params);
}
