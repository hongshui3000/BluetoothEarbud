...
{
    return (anc_mic_params.enabled_mics == hybrid_mode);
}
...
static audio_channel getAudioChannel(anc_path_enable mic_path)
{
    ...
    switch(mic_path)
    {
        ...
        case feed_forward_right:
            ...
            break;
        case feed_back_left:
            if(anc_mic_params.enabled_mics == hybrid_mode
...
static audio_mic_params getMonitorMic(void)
{
    audio_mic_params monitor_mic = { .bias_config = BIAS_CONFIG_DISABLE, .pre_amp = 0,
            .pio = 0, .gain = NON_ZERO_GAIN, .is_digital = FALSE, .instance = 0 };
...
{
    Source sync_source = AudioPluginGetMicSource(getMonitorMic(), AUDIO_CHANNEL_A);
...
{
    StreamConnect(StreamUsbEndPointSource(end_point_iso_in), ChainGetInput(tuning_chain, stream_usb));

    StreamConnect(ChainGetOutput(tuning_chain, stream_usb), StreamUsbEndPointSink(end_point_iso_out));
}
...
{
    disconnectSource(AudioPluginGetMicSource(getMonitorMic(), AUDIO_CHANNEL_A));

    disconnectSource(AudioPluginGetMicSource(getMonitorMic(), AUDIO_CHANNEL_B));
}
