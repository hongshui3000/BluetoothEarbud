...
uint16 anc_tuning_set_parameters(ANC_TUNING_OP_DATA *p_ext_data, ACCMD_ANC_INSTANCE instance_id)
{
    ...
    */
    if(is_ffa_en)    
    {
        ...
        anc_tuning_config_connected_to_ep(ep_ffb,ACCMD_CONFIG_KEY_STREAM_ANC_SM_LPF_FILTER_ENABLE,anc_inst_ptr->OFFSET_SMLPF_ENABLE);
        anc_tuning_config_connected_to_ep(ep_ffb,ACCMD_CONFIG_KEY_STREAM_ANC_SM_LPF_FILTER_SHIFT,anc_inst_ptr->OFFSET_SM_LPF_SHIFT);
        ...
        for (i=0;i<ACCMD_ANC_IIR_FILTER_FFA_NUM_COEFFS;i++)
        {
            /* capability XML format is signed Q2.N. ANC HAL is expecting signed Q2.9 uint16's. However, filter design utility is expecting a Q19.13 
               so >>20 and <<16 for coefficients to end up being shaped correctly for the ANC hardware 
            */
            p_ext_data->coeffs[i] = (uint16)((anc_inst_ptr->OFFSET_ANC_FF_A_COEFF[i]>>4)); 
        }
    ...
    if(is_ffb_en)
    {
        ...
        for (i=0;i<ACCMD_ANC_IIR_FILTER_FFB_NUM_COEFFS;i++)
        {
            /* capability XML format is signed Q2.N. ANC HAL is expecting signed Q2.9 uint16's. However, filter design utility is expecting a Q19.13 
               so >>20 and <<16 for coefficients to end up being shaped correctly for the ANC hardware 
            */
            p_ext_data->coeffs[i] = (uint16)((anc_inst_ptr->OFFSET_ANC_FF_B_COEFF[i]>>4)); 
        }
    ...
    if(is_fb_en)
    {
        ...
        for (i=0;i<ACCMD_ANC_IIR_FILTER_FB_NUM_COEFFS;i++)
        {
            /* capability XML format is signed Q2.N. ANC HAL is expecting signed Q2.9 uint16's. However, filter design utility is expecting a Q19.13 
               so >>20 and <<16 for coefficients to end up being shaped correctly for the ANC hardware 
            */
            p_ext_data->coeffs[i] = (uint16)((anc_inst_ptr->OFFSET_ANC_FB_COEFF[i]>>4)); 
        }
