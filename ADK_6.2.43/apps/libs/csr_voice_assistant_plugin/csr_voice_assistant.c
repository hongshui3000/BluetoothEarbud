...
void VoiceAssistantPluginStartCapture(AUDIO_PLUGIN_START_VOICE_CAPTURE_MSG_T *msg)
{
    ...
    /* Set the VA plugin for audio library*/
    AudioSetVoiceAssistantTask((Task)&voice_assistant_plugin);
}
...
void VoiceAssistantPluginStopCapture(void)
{
    ...
    /* Set the VA plugin for audio library*/
    AudioSetVoiceAssistantTask((Task)NULL);

        /* Audio busy to be cleared after the DSP is Unloaded successfully*/
        SetAudioBusy(NULL);
    }
}
