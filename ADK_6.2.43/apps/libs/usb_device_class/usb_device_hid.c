...
static bool usbEnumerateHidDatalinkControl(void)
{
    ...
    (void) VmalMessageSinkTask(StreamUsbClassSink(intf), &HID_DATA_TASK);
...
static void hidDatalinkHandler(Task task, MessageId id, Message message)
{
    ...
    if (id == MESSAGE_MORE_DATA)
    {
        ...
        handleHidClassRequest(StreamUsbClassSource(HID_DATA_INTF),
                                                       ...
                                                       uint8 report_id)
{
    ...
    sink = StreamUsbEndPointSink(HID_DATA_TO_HOST_ENDP_ADDR);

    if( size > USB_RESPONSE_MAX_SIZE ) size = USB_RESPONSE_MAX_SIZE;

    if ((out = claimSink(sink, sizeof(hid_response_msg_t))) != 0)
    {
        memset( out, 0x00, sizeof( hid_response_msg_t ) );
        ...
        if(!SinkFlush(sink, sizeof(hid_response_msg_t)))
        {
