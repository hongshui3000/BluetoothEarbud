...
static void mixerHardwareConnect(connection_type_t connection_type, uint32 sample_rate)
{
    ...
    hw_connect_data.primary_left = ChainGetOutput(mixerSpeakerGetChain(), left_channel);
    hw_connect_data.primary_right = ChainGetOutput(mixerSpeakerGetChain(), right_channel);
    hw_connect_data.secondary_left = ChainGetOutput(mixerSpeakerGetChain(), secondary_left_channel);
    hw_connect_data.secondary_right = ChainGetOutput(mixerSpeakerGetChain(), secondary_right_channel);
    hw_connect_data.wired_sub =  NULL;
...
static void mixerCreateAndConnect(const audio_mixer_connect_t* connect_data)
{
    ...
    if(!mixer_already_created)
    {
        ...
        mixerChainJoin(mixerCoreGetChain(), mixerSpeakerGetChain());
