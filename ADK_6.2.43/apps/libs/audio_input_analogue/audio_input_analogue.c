...
static audio_input_context_t *ana_ctx = 0;
...
static void destroyAnaCtx(Task task, Message message, audio_input_context_t* ctx)
{
    UNUSED(task);
    UNUSED(message);
    UNUSED(ctx);
...
{
    {
        ...
        [audio_input_error_state] = {[audio_input_disconnect_req] = destroyAnaCtx}
    }
};
