...
class ImageBuilder(object):
    ...
    default_flash0 = {
        ...
        "layout": [
            ...
            # Needs to be big enough to hold offloaded Audio constants
            # (DSPSW-818), currently ~48kbyte
        ...
        for name,params in self.config["layout"]:
            ...
            if "src_file" in params:
                ...
                if image:
                    ...
                    # If a capacity was requested along with a src_file then
                    # use the larger of the src_file size and the requested
                    # capacity for the actual section capacity.
                    section["capacity"] = max(section["size"],
                                              params.get("capacity", 0))
                else:
                    section["capacity"] = params["capacity"]
...
class ImageHeader(object):
    ...
    def build(self, sections, round_size=4096, nonce=None):
        ...
        if len(misplaced_sections):
            raise RuntimeError("Image sections:%s have to be placed before the Apps P0 section in the input flash_layout_config" %
