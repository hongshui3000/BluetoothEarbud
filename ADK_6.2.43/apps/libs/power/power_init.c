...
void PowerInit(Task clientTask, const power_config *config, const power_pmu_temp_mon_config *pmu_mon_config)
{
    if(config && !power)
    {
        ...
        if (pmu_mon_config != NULL)
        {
            ...
            power->calculated_external_resistance_in_milli_ohms = 
               PowerChargerExternalModeResistance(power->config.vchg.ext_resistor_value, power->config.vchg.ext_resistor_tolerance);
