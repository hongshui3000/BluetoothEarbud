...
/*! \brief Tone volume when no A2DP or SCO active, in dB */
...
#define appUiHfpVolumeLimit() \
    appUiPlayTone(app_tone_volume_limit)

/*! \brief Play HFP SLC connected tone */
#define appUiHfpConnected(silent) \
    { if (!(silent)) appUiPlayTone(app_tone_hfp_connected); }

/*! \brief Play HFP SLC disconnected tone */
...
/*! \brief Show pairing active LED pattern */
...
/*! \brief Play pairing complete tone */
...
extern void appUiPlayTone(const ringtone_note *tone);
extern void appUiPlayToneInterruptible(const ringtone_note *tone);
...
extern void appUiPowerOff(void);
#endif
