...
/*  Function that decides whether the earbud should be allowed to enter
    the low power mode - dormant.

    Dormant will lose all links so cannot be reached by the handset 
    until it is woken by motion (accelerometer), the button or power 
    (insertion into case).

    Blocks power down if the physical state is #PHY_STATE_IN_EAR or 
    the charger appears connected (regardless of charging state).

    \todo: Review whether there is scope for blocking dormant in 
        the case of #PHY_STATE_IN_CASE, where we have access to 
        battery, but may be little need for a connection to the 
        handset. Perhaps upgrade. Also #PHY_STATE_OUT_OF_EAR, which
        may indicate only a short time out of ear.

    \returns FALSE if there is any reason to block dormant, TRUE otherwise
*/
#ifdef INCLUDE_POWER_CONTROL
static bool appAllowPowerdownDecider(void)
{
...
{
#ifdef INCLUDE_POWER_CONTROL
    appPowerRegisterPowerDownCheck(appAllowPowerdownDecider);
#endif
...
#if 0
extern void appPeerPair(void);
extern void appHandsetPair(void);

/*! @brief Test function to initiate peer pairing, via main app state machine.
 
    May later get called from UI and promoted from test function status.
 */
void appPeerPair(void)
{
    if (appGetState() == APP_STATE_IDLE)
        appSetState(APP_STATE_PEER_PAIRING);
    else
        DEBUG_LOGF("appPeerPair only works in IDLE state not %d\n", appGetState());
}

/*! @brief Test function to initiate handset pairing, via main app state machine.
 
    May later get called from UI and promoted from test function status.
 */
void appHandsetPair(void)
{
    if (appGetState() == APP_STATE_IDLE)
        appSetState(APP_STATE_HANDSET_PAIRING);
    else
        DEBUG_LOGF("appHandsetPair only works in IDLE state not %d\n", appGetState());
}
#endif
