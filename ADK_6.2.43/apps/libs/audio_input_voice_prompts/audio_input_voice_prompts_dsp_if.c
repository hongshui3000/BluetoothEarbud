...
static vp_handle_t tones_data = NULL;

static bool isItToneEndMessage(MessageFromOperator *op_msg);
...
void VoicePromptsDspPlay(vp_context_t *context)
{
    ...
    {
        PRINT(("tone\n"));
        tones_data = VoicePromptsChainConnectTone(context->tone);
    }
    else
    {
        PRINT(("prompt\n"));
        tones_data = VoicePromptsChainConnectPrompt(context);
    }

    PRINT(("chain start\n"));
    VoicePromptsChainStart(tones_data);
}
...
{
    VoicePromptsChainStop(tones_data);
}
...
void VoicePromptsDspCleanup(vp_context_t *context)
{
    ...
    VoicePromptsChainDisconnect(tones_data);
    tones_data = NULL;
...
void VoicePromptsDspMessageHandler(vp_context_t *context, Task task, Message message)
{
    ...
    if(isItToneEndMessage((MessageFromOperator *)message))
    {
