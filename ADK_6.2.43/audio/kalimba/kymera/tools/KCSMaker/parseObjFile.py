    ...
    while i < len(lines):
        ...
        if m is not None:
            ...
            if m is not None:
                ...
                if var.has_key(name) or const.has_key(name):
                    raise Exception("%s: declared twice" % name)
                if prog.has_key(name):
    ...
    while i < len(lines):
        ...
        if m is not None:
            ...
            if not funcname.startswith("$") and not originalName.startswith("$"):
                # Use % as a separator between file name and variable name
                funcname = filename_obj + "%" + funcname
            if func.has_key(funcname) == False:
...
def parseMappingsKalscramble(mappings, lines):
    for l in lines:
        ...
        if m != None:
            ...
            logging.debug("kalscramble:" + name_left)
            logging.debug("kalscramble:" + name_right)
...
def getVariables(files, objDumpPath, mappings, func_import, kdc_maker):
    ...
    for func_dict in func_export:
        ...
        func_export[func_dict] = dict((os.path.basename(k.split("%")[0]) + "%" + k.split("%")[1] if k.find("%") != -1 else k, v) for k, v in func_export[func_dict].iteritems())
...
if __name__ == "__main__":
    ...
    for (name, syms) in (("Program", prog), ("Variables", var), ("Constant", const)):
        ...
        regions = list( set( map( lambda s: syms[s].region, syms ) ) )
        res += ", ".join(regions) + "\n"
