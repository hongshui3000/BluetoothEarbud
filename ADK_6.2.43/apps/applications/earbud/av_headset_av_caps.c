...
/*!@{ \name TWS sink endpoints (mono)
    \brief Predefined endpoints for audio Sink end point configurations, for receiving mono audio */
    /*! APTX */
...
/*!@{ \name TWS source endpoints (mono)
    \brief Predefined endpoints for audio Source end point configurations, for forwarding mono audio */
    /*! APTX */
const sep_config_type av_aptx_mono_tws_src = {AV_SEID_APTX_MONO_TWS_SRC, ENCODE_RESOURCE_ID, sep_media_type_audio, a2dp_source, FALSE, 0, sizeof(aptx_tws_caps), aptx_tws_caps};
    /*! SBC */
const sep_config_type av_sbc_mono_tws_src  = {AV_SEID_SBC_MONO_TWS_SRC,  ENCODE_RESOURCE_ID, sep_media_type_audio, a2dp_source, FALSE, 0, sizeof(sbc_tws_caps), sbc_tws_caps};
/*!@} */

/*! \brief Update sample rate in an SBC Mono TWS capability

    \param  caps        Pointer to a capability record. Such as retrieved by \ref appA2dpFindServiceCategory.
    \param  sample_rate Rate we want to set.
 */
void appAvUpdateSbcMonoTwsCapabilities(uint8 *caps, uint32_t sample_rate)
{
    caps[12] &= ~(SBC_SAMPLING_FREQ_48000 | SBC_SAMPLING_FREQ_44100);
    switch (sample_rate)
    {
        case 48000:
            caps[12] |= SBC_SAMPLING_FREQ_48000;
            break;

        case 44100:
            caps[12] |= SBC_SAMPLING_FREQ_44100;
            break;
...
{
    caps[8] &= ~(APTX_SAMPLING_FREQ_48000 | APTX_SAMPLING_FREQ_44100);
    switch (sample_rate)
    {
        case 48000:
            caps[8] |= APTX_SAMPLING_FREQ_48000;
            break;

        case 44100:
            caps[8] |= APTX_SAMPLING_FREQ_44100;
            break;
