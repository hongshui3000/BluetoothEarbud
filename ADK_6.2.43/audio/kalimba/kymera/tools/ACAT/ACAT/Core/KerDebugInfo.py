...
class KerDebugInfo(di.DebugInfoInterface):
    ...
    def read_debuginfo(self, build_output_path):
        ...
        """
        # Locate build output
        self.sym_path = os.path.normpath(build_output_path + '.sym')
        ...
        with cu.global_options.ker.scoped_ker(self.elf_path, ["debug_strings"]) \
            ...
            self.pm_rom = {}
            for addr, srcline in kal_elf_reader.get_statements().iteritems():
            ...
            self.pm_rom_by_name = {}
            for label in self.pm_rom.itervalues():
            ...
            for var_name, var in kal_elf_reader.get_variables().iteritems():
                # New Variable object.
            ...
            for name, info in self.section_hdrs.iteritems():
    ...
    def get_cap_data_type(self, cap_name, elf_id=None):
        ...
        """
        if not elf_id is None and self.elf_id != elf_id:
    ...
    def get_instruction(self, address):
        ...
        # do you.
        try:
            if self.get_pm_encoding(address) == 2:
    ...
    def get_type_info(self, type_name_or_id, elf_id=None):
        ...
        * The elf id of this debug information.
        ...
        elif isinstance(type_name_or_id, (int, long)):
        ...
        if not matched_types:
            ...
            # the same name. References always work backwards, so pick the
            # one with the highest number.
            first_tid = max([t.type_id for t in matched_types])
        else:
    ...
    def inspect_var(self, var, elf_id=None):
        ...
        # If this is an array, split the variable up into individual elements.
        if var.array_len is not None:
            ...
            for count in range(var.array_len):
                ...
                member_value = var.value[
                    (member_addr - var.address) / Arch.addr_per_word:int(
                        math.ceil(
                            (
                                member_addr - var.address + element_size
                            ) / float(Arch.addr_per_word)
                        )
                    )
                ]  # Slice of the variable
                # name, address, size, value = None, type = None, members =
                # None, parent = None
                new_var = ct.Variable(
                    name=member_name,
                    address=member_addr,
        ...
        # If the type has members (i.e. it's a union or a struct), AND it's not
        # an array or pointer, go through the members.
        if members_typeid is not None:
            ...
            for mem in members:
                ...
                if mem.bit_offset_from_offset != 0xFFFFFFFF:
                    ...
                    # (Arch4 specific)
                    if member_size < 4 and Arch.addr_per_word == 4:
                        value = var.value[mem.offset / Arch.addr_per_word:
                                          mem.offset / Arch.addr_per_word + 1]
                        if value:
                            # the list has only one value anyway
                            value = value[0]
                            ...
                            value = value >> (
                                8 * (mem.offset % Arch.addr_per_word)
                            )
                        ...
                        member_value = var.value[mem.offset / Arch.addr_per_word:(
                            mem.offset / Arch.addr_per_word + int(
                                math.
                                ceil(member_size / float(Arch.addr_per_word))
                            )
                        )]  # Slice of the variable
                ...
                # Cash-up. We don't generally ever want to add union
                # members to var_by_addr, but they should be excluded by
    ...
    def get_pm_encoding(self, address):
        """
        @brief Look up the contents of Program Memory at the supplied address, and
        work out whether it is encoded as Maxim or Minim.
        Return value is one of {1 (Maxim), 2 (Minim)}. Exception
        UnknownPmEncoding is raised if the encoding is unknown
        @param[in] self Pointer to the current object
        @param[in] address
        @param[out] Pm encoding
        """
        ...
        pm_encoding = 0  # unknown
        for _, info in self.pm_section_hdrs.iteritems():  # (name, info)
    ...
    def _acquire_debug_strings(self):
        ...
        if Arch.addr_per_word == 4:
            ...
            for addr, entry in sorted(self.dm_const.iteritems()):
