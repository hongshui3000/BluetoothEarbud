...
{
    MAKE_OPERATOR_CONFIG_WITH_SETUP(capability_id_usb_audio_rx, audio_voice_receive_role, usb_setup_nb), \
...
{
    MAKE_OPERATOR_CONFIG_WITH_SETUP(capability_id_usb_audio_rx, audio_voice_receive_role, usb_setup_wb), \
    MAKE_OPERATOR_CONFIG_WITH_SETUP(capability_id_usb_audio_tx, audio_voice_send_role, usb_setup_wb)
};

/******************************************************************************/

static bool isWideband(unsigned encoder)
{
...
void AudioInputUsbConnectAndFadeIn(Task task, Message msg, audio_voice_context_t* ctx)
{
    ...
    operator_filters_t* filters = getFilters(isWideband(plugin->encoder));
