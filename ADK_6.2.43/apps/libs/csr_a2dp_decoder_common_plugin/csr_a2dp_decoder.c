...
/* timeout period before we consider the DSP patch process completed. 
   6ms   was found to be reliable. This was doubled to 12ms to give a safty margin*/
#define WAIT_FOR_NEXT_PATCH_REQUEST_MILLISEC (12)
...
#define GET_PARAMS_PAYLOAD_START 2
#define NUMBER_OF_PARAMS_OFFSET 0
#define PARAMS_OFFSET 1
    ...
    if (decoder)
    {
        if (result == decoder->stream_relay_mode)
        {
            ...
            {   /* Only set routing mode if operating as a TWS Master */
                PRINT(("DECODER: Issuing DSP with MESSAGE_SET_TWS_ROUTING=%u,%u\n",AudioConfigGetTwsChannelModeLocal(), AudioConfigGetTwsChannelModeRemote()));

                KALIMBA_SEND_MESSAGE(MESSAGE_SET_TWS_ROUTING, AudioConfigGetTwsChannelModeLocal(), AudioConfigGetTwsChannelModeRemote(), 0, 0);
...
static uint16 getMusicManagerSampleRate(void)
{
    DECODER_t* decoder = CsrA2dpDecoderGetDecoderData();
    A2dpPluginConnectParams * codecData = (A2dpPluginConnectParams *)decoder->params;

    uint16 music_manager_rate = decoder->dsp_resample_rate;

    if(codecData->sub_functionality_present)
    {
        /* Fix output sampling rate to allow re-sampling of I2S outputs */
        music_manager_rate = (FIXED_SUBWOOFER_DAC_SAMPLE_RATE/DSP_RESAMPLING_RATE_COEFFICIENT);
    }
    return music_manager_rate;
}
...
DESCRIPTION
    ...
    Sends input ('CODEC') and output sample rate configuration messages
    to the DSP, taking into account any mismatch.
*/
void audioDecodeSendDspSampleRateMessages(local_config_t* localConfig)
{
    KALIMBA_SEND_MESSAGE(MESSAGE_SET_DAC_OUTPUT_RESAMPLER_MSG,
                                        getDacOutputResamplingMode(), 0, 0, 0);

    KALIMBA_SEND_MESSAGE(MESSAGE_SET_MUSIC_MANAGER_SAMPLE_RATE,
                                getMusicManagerSampleRate(),
                                localConfig->mismatch,
                                localConfig->val_clock_mismatch,
                                CsrA2dpDecoderPluginIfLowLatencyGetOutputType());

    KALIMBA_SEND_MESSAGE(MESSAGE_SET_CODEC_SAMPLE_RATE,
                                (uint16)(CsrA2dpDecoderGetDecoderData()->rate/DSP_RESAMPLING_RATE_COEFFICIENT),
                                16000/DSP_RESAMPLING_RATE_COEFFICIENT, 0, 0);
...
void audioDecodePlayTone(A2dpPluginTaskdata *task, const ringtone_note * tone)
{
    ...
    if (decoder)
    {
        ...
        KALIMBA_SEND_MESSAGE(MESSAGE_SET_TONE_RATE_MESSAGE_ID, 8000 , MONO_PROMPT | NO_EXTRA_GAIN , 0, 0);
                ...
                case KALIMBA_MSG_SOURCE_CLOCK_MISMATCH_RATE:
                    {
                        if (decoder)
                        {
                            ...
                            MessageSend(decoder->app_task, AUDIO_DSP_IND, dsp_ind_message);
                        }
                    }
                    break;
                    ...
                    */
                    {
                        if (decoder)
                        {
                            MAKE_AUDIO_MESSAGE_WITH_LEN(AUDIO_DSP_IND, 2, dsp_ind_message);
                ...
                case MUSIC_SIGNAL_DETECTOR_STATUS_RESP:
                    ...
                    */

                    {
                        ...
                        {
                            MAKE_AUDIO_MESSAGE(AUDIO_SIGNAL_DETECT_MSG, signal_detect_message);
                            signal_detect_message->signal_detected = signal;
                            MessageSend(decoder->app_task, AUDIO_SIGNAL_DETECT_MSG, signal_detect_message);
                        }
...
#define PARAMS_OFFSET 1
        ...
        case MESSAGE_FROM_KALIMBA_LONG:
        {
            ...
            switch ( rcv_msg[0] )
            {
                case KALIMBA_GET_USER_EQ_PARAMS_RESP:
                    {
                        unsigned i;
                        const uint16 number_of_params = rcv_msg[GET_PARAMS_PAYLOAD_START + NUMBER_OF_PARAMS_OFFSET];
                        size_t message_size = CALC_MESSAGE_LENGTH_WITH_VARIABLE_PARAMS(AUDIO_GET_USER_EQ_PARAMETERS_CFM_T,
                                                                                         number_of_params,
                                                                                         user_eq_param_t);

                        AUDIO_GET_USER_EQ_PARAMETERS_CFM_T* user_eq_message = PanicUnlessMalloc(message_size);
                        memset(user_eq_message, 0, message_size);
                        PRINT(("DECODER: User EQ Group Param from DSP: [%x][%x]...\n", rcv_msg[2],rcv_msg[3]));

                        user_eq_message->data_valid = TRUE;
                        user_eq_message->number_of_params = number_of_params;

                        for(i = 0; i < number_of_params; i++)
                        {
                            unsigned offset = GET_PARAMS_PAYLOAD_START + PARAMS_OFFSET + (i * NUM_WORDS_PER_PARAM);
                            uint16 param = rcv_msg[offset + ID_OFFSET];
                            user_eq_message->params[i].id.bank = (uint16)((param >> 8) & 0x000f);
                            user_eq_message->params[i].id.band = (uint16)((param >> 4) & 0x000f);
                            user_eq_message->params[i].id.param_type = (uint16)(param & 0x000f);
                            user_eq_message->params[i].value = rcv_msg[offset + VALUE_OFFSET];
                        }
                        MessageSend(decoder->app_task, AUDIO_GET_USER_EQ_PARAMETERS_CFM, user_eq_message);
                    }
                    break;
                ...
                case KALIMBA_MSG_FILE_NAME:
                {
                    const MessageFromKalimbaLong *m = (const MessageFromKalimbaLong *) message;
                    const char *filename = (const char *) m->data;
                    FILE_INDEX index = FileFind(FILE_ROOT, filename, (uint16) strlen(filename));

                    PRINT(("KALIMBA_MSG_FILE_NAME: \"%s\" -> %u\n", filename, index));
                    KalimbaSendMessage(KALIMBA_MSG_FILE_INDEX, index, 0, 0, 0);

                    /* Cancel any DSP Patch End timer */
                    (void) MessageCancelAll( (TaskData*) decoder->task, AUDIO_PLUGIN_DSP_PATCH_PROCESS_END_MSG);
                    /* Start a timer message to signal the end of the patch process */
                    MessageSendLater( (TaskData*) decoder->task, AUDIO_PLUGIN_DSP_PATCH_PROCESS_END_MSG, NULL, WAIT_FOR_NEXT_PATCH_REQUEST_MILLISEC);
                }
                break;
...
void audioDecodePluginUpdateEnhancements(const A2dpPluginModeParams *mode_params)
{
    ...
    if(mode_params)
    {
        ...
        {
            PRINT(("DECODER: Set Audio Enhancements Configuration [%x]\n" , mode_params->music_mode_enhancements));
...
DESCRIPTION
    set relay control
*/
void audioDecodeDspSendRelayControl(uint16 amount, uint16 flush_threshold)
{
    KALIMBA_SEND_MESSAGE(KALIMBA_MSG_RELAY_CONTROL, amount, flush_threshold, 0, 0);
}
...
/****************************************************************************
DESCRIPTION
    Connects Analogue Input to dsp
*/
void audioDecodeDspConnectAnalogueInputToDsp(Source l_source, Source r_source, A2dpPluginConnectParams* codecData)
{
    DECODER_t* decoder = CsrA2dpDecoderGetDecoderData();
    l_source = AudioPluginAnalogueInputSetup(AUDIO_CHANNEL_A, *codecData->analogue_in_params, decoder->rate);
    ...
    r_source = AudioPluginAnalogueInputSetup(AUDIO_CHANNEL_B, *codecData->analogue_in_params, decoder->rate);
    /* Specify the bits per sample for the audio input */
