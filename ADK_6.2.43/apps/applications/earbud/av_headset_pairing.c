...
#define COPY_PAIRING_MESSAGE(src, dst) *(dst) = *(src);
...
static void appPairingInitialiseEir(const uint8 *local_name, uint16 size_local_name)
{
    ...
    static const uint8 eir_128bit_uuids[] =
    {
        ...
        0x9D, 0x30, 0x3E, 0x04, 0x69, 0x75, 0x68, 0xAE, 0x33, 0x49, 0xEB, 0xE4, 0xB5, 0x55, 0x00, 0x76
    };
...
bool appPairingHandleClSmAuthoriseIndication(const CL_SM_AUTHORISE_IND_T *ind)
{
    ...
    if ((ind->protocol_id == protocol_l2cap) && (ind->channel == 1) && ind->incoming && thePairing->sdp_lock)
    {
        ...
        COPY_PAIRING_MESSAGE(ind, message);
...
/*! \brief Handle Security mode confirmation */
...
/*! \brief Handle Pin code indication
...
/*! \brief Handle user passkey confirmation
...
/*! \brief Handle user enter passkey indication
...
void appPairingInit(void)
{
     ...
     * we will may get PEER_SIG_CHANNEL_IND messages if the signalling
