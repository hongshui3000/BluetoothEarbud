...
/**
 * \file
 * USB device class library header
 */

#ifndef USB_DEVICE_CLASS_H
#define USB_DEVICE_CLASS_H
...
/**
 * Possible return status of a called API. */
typedef enum
{
    ...
    /** The enumeration of the USB device was unsuccessful.
     * The Host Interface should be checked that it is set to USB. */
...
/**
 * Possible USB device classes that can be handled by the library.
 * These values can be passed into the UsbDeviceClassEnumerate API.
 */
typedef enum
{
...
/**
 * Possible configuration values that can be passed
 * into the UsbDeviceClassConfigure API.
 */
typedef enum
{
    ...
    /** Configure audio streaming/control interface descriptors
     * (see usb_device_class_audio_config) */
    USB_DEVICE_CLASS_CONFIG_AUDIO_INTERFACE_DESCRIPTORS,
    /** Configure HID consumer transport interface descriptors
     * (see usb_device_class_hid_consumer_transport_config) */
...
/**
 * Possible values that can be returned in the value parameter
 * of the UsbDeviceClassGetValue API. These values will only be returned
 * if USB_DEVICE_CLASS_GET_VALUE_CHARGER_STATUS was passed into the API.
 */
typedef enum
{
...
/**
 * Possible values that can be passed into the UsbDeviceClassSendEvent API.
 * Events are only valid if the correct device class has been enumerated.
 */
typedef enum
{
...
/**
 * Structure to pass when calling UsbDeviceClassGetValue
 * to obtain USB_DEVICE_CLASS_GET_VALUE_AUDIO_LEVELS
 */
typedef struct
{
    ...
    int16      user_audio_levels;
} usb_device_class_audio_levels;


/**
 * Structure to hold interface descriptor. Descriptors must be constant,
 * or if allocated may not be freed.
 */
typedef struct
{
...
/**
 * Structure to pass as params when calling UsbDeviceClassConfigure
 * to set USB_DEVICE_CLASS_CONFIG_AUDIO_INTERFACE_DESCRIPTORS.
 * Interface descriptors must be set prior to calling UsbDeviceClassEnumerate
 * if the application wishes to use custom descriptors, otherwise defaults
 * will be used. This structure must contain all descriptors required for
 * the enumerated audio device classes.
 * Terminal and unit ID's must match those used by the library,
 * i.e. Microphone(4) -> Feature Unit(5) -> USB Output Terminal(6)
...
/**
 * Structure to pass as params when calling UsbDeviceClassConfigure
 * to set USB_DEVICE_CLASS_CONFIG_HID_CONSUMER_TRANSPORT_DESCRIPTORS.
 * Interface descriptors must be set prior to calling UsbDeviceClassEnumerate
 * if the application wishes to use custom descriptors, otherwise
...
/**
 * This message is returned when the battery and charger monitoring
 ...
 *
 * Could be called before or after UsbDeviceClassEnumerate,
 ...
 *
 * Some combination of classes maybe invalid in which case the function
 * would return failure and no enumeration would be performed.
 ...
 * For battery charging or mic/speaker enumeration this should be
 * the system Task which receives all MESSAGE_USB_xxx messages.
 * For mic/speaker enumeration MESSAGE_USB_ALT_INTERFACE can be used
 * to determine if audio should be routed. The interface ID
 * for mic/speaker can be obtained using UsbDeviceClassGetValue().
 * If an alternate setting of 1 (audio streaming) is selected
 * for these interfaces then audio is present, if an alternate setting of 0
 * (audio control) is selected then audio is not present.
 * \param usb_device_class One or more of the values from usb_device_class_type,
 * indicating which USB device classes to enumerate as.
*/
 ...
 */
usb_device_class_status UsbDeviceClassGetValue(usb_device_class_get_value id,
                                               uint16 *value);
 ...
 *
 * \param event The event to send over USB. Events will only be sent
 * if the correct device class has been enumerated.
 */
 ...
 *
 * \param class_type One of the values from usb_device_class_type,
 * indicating which USB device class to send the report for.
