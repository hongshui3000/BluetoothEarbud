/**
 * \file
 * VM library module that implements USB HID class device support
 */
...
#include "usb_device_class/usb_device_class.h"
#include "usb_device_class/usb_device_class_private.h"
#include "usb_device_class/usb_device_hid.h"
...
usb_hid_consumer_config = NULL;

static const UsbCodes usb_codes_hid_no_boot =
    {B_INTERFACE_CLASS_HID, /* bInterfaceClass */
...
static const usb_device_class_hid_consumer_transport_config
usb_hid_config_consumer_transport =
{
...
static bool usbEnumerateHidConsumerTransportControl(void)
{
    ...
    if (!usb_hid_consumer_config)
    {
        ...
        L3_DBG_MSG("USB: HID consumer default descriptors");
    }
    ...
    (void) MessageStreamTaskFromSink(StreamUsbClassSink(intf), &HID_CONS_TASK);
...
static bool usbEnumerateHidKeyboard(bool consumer_active)
{
    ...
    (void) MessageStreamTaskFromSink(StreamUsbClassSink(intf), &HID_KEYB_TASK);
...
static void hidConsumerHandler(Task task, MessageId id, Message message)
{
    ...
    {
        L3_DBG_MSG("USB: MESSAGE_MORE_DATA hid consumer");
...
static void hidKeyboardHandler(Task task, MessageId id, Message message)
{
    ...
    if (id == MESSAGE_MORE_DATA)
    {
        L3_DBG_MSG("USB: MESSAGE_MORE_DATA hid keyboard");
                                  ...
                                  usb_device_class_type class_type)
{
    ...
    {    
        ...
        while ((packet_size = SourceBoundary(source)) != 0)
        {
            ...
            UsbResponse resp;
            memcpy(&resp.original_request, SourceMapHeader(source),
                   sizeof(UsbRequest));
        
            /* Set the response fields to default values to make
            ...
            switch (resp.original_request.bRequest)
            {
                ...
                case 0x01:
                {
                    L3_DBG_MSG4("USB: HID Get_Report src=0x%X wValue=0x%X "
                ...
                case 0x02:
                {
                    ...
                    {
                        L3_DBG_MSG3("USB: HID Get_Idle src=0x%X "
                ...
                case 0x09:
                {
                    ...
                    L3_DBG_MSG4("USB: HID Set_Report src=0x%X wValue=0x%X "
                                "wIndex=0x%X wLength=0x%X -> ",
                                source,
                                resp.original_request.wValue,
                                resp.original_request.wIndex,
                    ...
                    if (size_data)
                    {
                           ...
                           USB_DEVICE_CLASS_TYPE_HID_CONSUMER_TRANSPORT_CONTROL)
                        {
                            ...
                            L3_DBG_MSG1("    send report ind source_size[0x%x]",
                                    source_size);
                            ...
                            L3_DBG_MSG1("    sent report ind to Task [0x%x]",
                                        device->app_task);
                        }
                    }                                     
                    break;
                ...
                case 0x0A:    
                {
                    L3_DBG_MSG3("USB: HID Set_Idle src=0x%X "
                                "wValue=0x%X wIndex=0x%X",
                                source,
                                resp.original_request.wValue,
                                resp.original_request.wIndex);
                ...
                default:
                {
                    L3_DBG_MSG3("USB: HID req=0x%X src=0x%X wValue=0x%X",
                            resp.original_request.bRequest,
                            source, resp.original_request.wValue);
                    L3_DBG_MSG2("USB: HID wIndex=0x%X wLength=0x%X",
                            resp.original_request.wIndex,
                            resp.original_request.wLength);
                    break;            
                }
            ...
            if (resp.data_length)
            {
                (void)SinkFlushHeader(sink, resp.data_length,
                        (uint16 *)&resp, sizeof(UsbResponse));
            }
            else
            {
                /* Sink packets can never be zero-length,
                 * so flush a dummy byte */
                (void) SinkClaim(sink, 1);
                (void) SinkFlushHeader(sink, 1,
                        (uint16 *) &resp, sizeof(UsbResponse));
            }   
...
bool usbEnumerateHid(uint16 usb_device_class)
{
    ...
    {
        L3_DBG_MSG("USB: Enumerate consumer/keyboard");
        if (usbEnumerateHidConsumerTransportControl() &&
                usbEnumerateHidKeyboard(TRUE))
        {
            return TRUE;
        }
        else
        ...
        const usb_device_class_hid_consumer_transport_config* params)
{
    ...
    L3_DBG_MSG("USB: HID consumer configured to use supplied descriptors");
    
    return TRUE;
}


bool usbSendRawEventHidKeycode(usb_device_class_type type, uint16 size_params,
        const uint8 *params)
{
    uint8 *ptr;
    uint16 i;
...
usb_device_class_status usbSendDefaultHidConsumerEvent(
        usb_device_class_event event)
{
    switch (event)
    {
...
usb_device_class_status usbSendDefaultHidKeyboardEvent(
        usb_device_class_event event)
{
    switch (event)
    {
...
bool usbSendReportHidConsumerTransport(uint16 report_id, uint16 size_report,
        uint8 *report)
{
    ...
    if (data)
    {
        L3_DBG_MSG("USB: Send Report with data ");
        ...
        {                
            data[i + 1] = report[i];
            L3_DBG_MSG1("%x ", data[i]);
        }
        L3_DBG_MSG1("%x", data[size_report]);
