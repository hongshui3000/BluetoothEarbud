...
static uint16 voiceAssistantDspConvertEncoderFormat(void)
{
    ...
    uint16 format = (uint16)(AudioConfigGetSbcEncoderParams().sbc_encoder_format);
...
static void voiceAssistantDspSetEncoderParams(void){
    ...
    (void)PanicFalse(KalimbaSendMessage(KALIMBA_MSG_SBCENC_SET_BITPOOL, AudioConfigGetSbcEncoderParams().sbc_encoder_bitpool, 0, 0, 0));
}
...
static void  voiceAssistantDspDisconnectMicPorts(void){
    ...
    AudioPluginSetMicPio(configured_mic_params, FALSE);
...
static void voiceAssistantHandleMusicReadyMessage(void)
{
    ...
    if(context)
    {
        ...
        /* Audio busy to be cleared after the DSP is loaded successfully*/
        SetAudioBusy(NULL);
    }
    else
    {
