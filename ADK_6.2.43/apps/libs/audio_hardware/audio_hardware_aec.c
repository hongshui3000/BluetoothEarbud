...
{
    /* AEC is always required for voice connections. */
    if(connection_type == CONNECTION_TYPE_VOICE)
        return TRUE;

    if(connection_type == CONNECTION_TYPE_MUSIC)
    {
        /* AEC is only required for music if we support mixing of music and
           voice inputs. Note that if AEC is present Timed Playback will not
           work for music inputs. */
        if (AudioConfigGetMaximumConcurrentInputs() > 1)
            return TRUE;
    }

    /* All other connection types default to not requiring AEC. */
    return FALSE;
}
