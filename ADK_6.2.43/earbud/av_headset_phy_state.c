...
void appPhyStateRegisterClient(Task client_task)
{
    ...
    /* Initialise the module when first client registers */
    if (NULL == phy_state->client_tasks)
    {
...
void appPhyStateUnregisterClient(Task client_task)
{
    ...
    if (0 == appTaskListSize(phy_state->client_tasks))
    {
        /* Uninitialise the module when the final client unregisters */
        appProximityClientUnregister(&phy_state->task);
        appAccelerometerClientUnregister(&phy_state->task);
        appTaskListDestroy(phy_state->client_tasks);
        phy_state->client_tasks = NULL;
        phy_state->state = PHY_STATE_UNKNOWN;
    }
}
