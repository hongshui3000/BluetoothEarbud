 ...
 * \return PIO_CFG_SUCCESS if request was successful, PIO_CFG_BUSY if a previous
 * request was not finished yet, PIO_CFG_NO_BULK_QUERY indicates that the query
 * requested cannot be made for more than one PIO at a time,
 * PIO_CFG_NON_XIOS_IN_MASK indicates an XIO only operation requested for
 * non-XIO PIOs. The callback will be called only if the PIO_CFG_SUCCESS status
 * is returned.
 */
