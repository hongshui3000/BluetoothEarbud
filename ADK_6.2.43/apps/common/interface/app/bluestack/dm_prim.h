...
/*
 * DM_SM_PERMANENT_ADDRESS_T is now obsolete but we still need to have backward
 * compatibility.
 */
#define DM_SM_PERMANENT_ADDRESS_T DM_SM_RANDOM_ADDRESS_T

/*
   This API generates a random address of type requested and then sets this
   value to controller.
   In case static address is provided by application, it sets the same static
   random address to controller if valid. Otherwise generate a new one and sets
   to controller.

   Note:
   - Used to configure random address.
     TGAP(private_addr_int) timer shall be implemented in the application to
     generate new random address by calling into this API.
   - A new RPA is generated for every application request, provided Bluestack is
     not busy handling previous similar request. When busy, a descriptive error
     is indicated in its corresponding confirmation.
 */
typedef struct
{
    dm_prim_t type;                     /* DM_SM_SET_PRIVACY_REQ */
    DM_SM_RANDOM_ADDRESS_T set_permanent_address_type;/* random address type */
...
typedef struct
{
    ...
    uint8_t status;
    DM_SM_RANDOM_ADDRESS_T permanent_address_type; /* Type of random addess set to controller */
