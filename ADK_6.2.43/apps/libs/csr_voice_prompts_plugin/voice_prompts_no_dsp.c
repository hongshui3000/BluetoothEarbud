...
void VoicePromptsNoDspPlay(voice_prompts_codec_t codec_type,
        unsigned sample_rate, Source left_source, Source right_source)
{
    Sink lSink;
	...
	volume = (AudioConfigGetTonesAreFixedVolume() ? (int16)AudioConfigGetToneVolume() : (int16)AudioConfigGetMasterVolume());

    /* Set gain (no DSP so set main hardware gains only) */
    AudioOutputGainSetHardwareOnly(audio_output_group_main, volume);
}
