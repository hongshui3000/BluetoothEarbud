...
#define GAIA_UPGRADE_DATA_REQUEST (0x04)
#define GAIA_RWCP_UPGRADE_DATA_BYTES_REQ_OFFSET (0x06)
#define GAIA_RWCP_UPGRADE_DATA_BYTES_REQ (0x03)
#define GAIA_RWCP_UPGRADE_BUFFER_SIZE ( (UPGRADE_MAX_PARTITION_DATA_BLOCK_SIZE)  / (RWCP_UPGRADE_MAX_DATA_SIZE) ) * ( RWCP_UPGRADE_MAX_DATA_SIZE )
...
static bool set_data_endpoint_mode(gaia_transport *transport, uint8 mode)
{
    switch(mode)
    {
        case GAIA_DATA_ENDPOINT_MODE_NONE:
            ...
            UpgradeSetPartitionDataBlockSize(UPGRADE_MAX_PARTITION_DATA_BLOCK_SIZE);
            return TRUE;
        ...
        case GAIA_DATA_ENDPOINT_MODE_RWCP:
            ...
            UpgradeSetPartitionDataBlockSize(GAIA_RWCP_UPGRADE_BUFFER_SIZE);
            return TRUE;
...
static void upgrade_connect(gaia_transport *transport)
{
    if (gaia->upgrade_transport == NULL)
    {
        ...
        {
            RwcpServerInit(GAIA_RWCP_UPGRADE_BUFFER_SIZE, (GAIA_GATT_HEADER_SIZE + GAIA_UPGRADE_HEADER_SIZE));
            RwcpSetClientTask(&gaia->task_data);        
            UpgradeSetPartitionDataBlockSize(GAIA_RWCP_UPGRADE_BUFFER_SIZE);
        }
        else
...
/*----------------------------------------------------------------------------*
 *  NAME
 *      send_rwcp_notification
 *
 *  DESCRIPTION
 *      This function handles payloads sent from the RWCP server
 *
 *  RETURNS
 *      Nothing
 *
 *---------------------------------------------------------------------------*/
static void send_rwcp_notification(uint8 *payload, uint16 payload_length)
{
    GAIA_DEBUG(("send_rwcp_notification\n"));
    gaiaTransportGattRes(gaia->upgrade_transport, payload_length, payload, HANDLE_GAIA_DATA_ENDPOINT);
    free(payload);
}

/*----------------------------------------------------------------------------*
 *  NAME
 *      rwcp_process_command
 *
 *  DESCRIPTION
...
static void rwcp_process_command(uint8 *command, uint16 size_command)
{
...
*/
    ...
    case UPGRADE_TRANSPORT_DATA_IND:
        ...
        if (gaia->upgrade_transport && gaia->upgrade_transport->notify_vmup)
        {
            ...
            if (gaia->data_endpoint_mode ==  GAIA_DATA_ENDPOINT_MODE_RWCP)
            {
                ...
                RwcpHandleUpgradeTransportDataInd();

                if(UpgradeIsPartitionDataState() && data[0] == GAIA_RWCP_UPGRADE_DATA_BYTES_REQ)
                {
                    if((first == FALSE) && (ind->size_data > GAIA_RWCP_UPGRADE_DATA_BYTES_REQ_OFFSET))
                    {
                        if(ByteUtilsGet4BytesFromStream(&data[GAIA_RWCP_UPGRADE_DATA_BYTES_REQ_OFFSET - 3]) == GAIA_RWCP_UPGRADE_BUFFER_SIZE)
                        {
                            uint32 size = 0;

                            size = UpgradeGetPartitionSizeInPartitionDataState();
                            data[GAIA_RWCP_UPGRADE_DATA_BYTES_REQ_OFFSET] = size & 0xFF;
                            data[GAIA_RWCP_UPGRADE_DATA_BYTES_REQ_OFFSET - 1] = ( size >> 8 ) & 0xFF;
                            data[GAIA_RWCP_UPGRADE_DATA_BYTES_REQ_OFFSET - 2] = ( size >> 16 ) & 0xFF;
                            data[GAIA_RWCP_UPGRADE_DATA_BYTES_REQ_OFFSET - 3] = ( size >> 24 ) & 0xFF;
                            first = TRUE;
                        }
    ...
    case UPGRADE_TRANSPORT_DATA_CFM:
        if (gaia->upgrade_transport)
        {
            ...
            if ((gaia->data_endpoint_mode ==  GAIA_DATA_ENDPOINT_MODE_RWCP) && cfm->size_data > 0 && cfm->data != NULL &&  cfm->data[0] == GAIA_UPGRADE_DATA_REQUEST)
                break;
    ...
    case RWCP_UPGRADE_DATA:
        {
            ...
            rwcp_process_command(ind->payload, ind->size_payload);
            free(ind->payload);
        }
        break;
    ...
    case RWCP_UPGRADE_DATA_ACK:
        {
            ...
            send_rwcp_notification(ind->payload, ind->size_payload);
        }        
        break;
...
{
    RwcpServerHandleMessage(data, size_data, UpgradeIsPartitionDataState());
    UNUSED(transport);
}
