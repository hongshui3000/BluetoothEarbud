...
/*! Microphone quality value of 'best'. */
#define AT_TWS_MIC_QUALITY_BEST     (15)
                                                 ...
                                                 AV_AVRCP_CONNECT_IND_T *ind)
{
    ...
    appAvAvrcpConnectResponse(&handset_sig->task, &ind->bd_addr, ind->connection_id,
                                              ...
                                              APP_HFP_AT_CMD_IND_T* ind)
{
    ...
    switch (type)
    {
        case AT_TYPE_MIC_QUALITY_REQ:
            ...
            /* we don't have a way to get current MIC quality from audio, so
             * just always send maximum quality value of 15. */
            appHandsetSigSendMicQualityReq(AT_TWS_MIC_QUALITY_BEST);
            break;
