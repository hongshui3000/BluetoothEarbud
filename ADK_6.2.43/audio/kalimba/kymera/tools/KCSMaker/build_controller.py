...
class Build_Controller(object):
    ...
    def write_linkscript_and_start_file(self, chip_name, start_name_list, linkscript_template):
        ...
        kdc_start_tmplt = open(self.kcsmaker_path + "/templates/kdc_start_tmpl")
        ...
        self.kdc_start_name = os.path.abspath(self.outputdir + "/kdc_start.asm")
        temp_scriptname = self.linkscript_name+".new_vsn"
        temp_startname = self.kdc_start_name+".new_vsn"

        if not os.path.exists(os.path.dirname(self.linkscript_path_name)):
            os.makedirs(os.path.dirname(self.linkscript_path_name))
        link_dnld = open(temp_scriptname, "w")

        for line in link_tmplt.xreadlines():
            if chip_name in ["crescendo", "stre", "napier"]:
        ...
        kdc_start = open(temp_startname, "w")

        for line in kdc_start_tmplt.xreadlines():
            if "r0" in line:
                line = ""
        ...
        self.replace_if_diff(self.linkscript_path_name, temp_scriptname)
        self.replace_if_diff(self.kdc_start_name, temp_startname)
    ...
    def write_makefile(self, cap_list, exe_name):
        ...
        for line in make_tmplt.xreadlines():
    ...
    def build(self, binfo, cap_list, exe_name, linkscript_template, make_cmd, clean_first=True):
        ...
        if clean_first:
            logging.debug("Starting clean. " + make_cmd + " --jobs=16 -s -f " + self.makefile_name + " CONFIG=" + self.download_config + " OSTYPE=" + self.os_type + "clean")
            clean = subprocess.call(make_cmd + " --jobs=16 -s -f " + self.makefile_name + " CONFIG=" + self.download_config + " OSTYPE=" + self.os_type + " clean", shell=True)
            if clean != 0:
                logging.error("\nClean build failed.\nExiting...")
                sys.exit(-1)
        logging.debug("Starting make. " + make_cmd + " --jobs=16 -s -f " + self.makefile_name + " CONFIG=" + self.download_config + " OSTYPE=" + self.os_type)
        make = subprocess.call(make_cmd + " --jobs=16 -s -f " + self.makefile_name + " CONFIG=" + self.download_config + " OSTYPE=" + self.os_type, shell=True)
