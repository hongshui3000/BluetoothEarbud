...
/**
 * Send a stream transaction to the host with firmware log information.
 * Currently only valid for processor 0 since there is only one hardware
 * instance and we don't want customers able to send arbitrary transactions.
 * Processor 1 on Crescendo D01 will gain extra hardware of a slightly
 * different shape to support this - Module: bus_proc_stream.
