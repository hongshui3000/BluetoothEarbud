...
void audioInputA2dpBroadcastCreate(Task input_task, Task output_task, audio_input_context_t *ctx)
{
    ...
    params.forwarding.pcm.left_source = ChainGetOutput(ctx->chain, stream_pcm_left);
    params.forwarding.pcm.right_source = ChainGetOutput(ctx->chain, stream_pcm_right);
    AudioPluginForwardingCreate(input_task, output_task, &params);
}

void audioInputA2dpBroadcastStart(Task input_task, audio_input_context_t *ctx, AUDIO_PLUGIN_FORWARDING_CREATE_CFM_T* cfm)
{
    if(cfm->status == audio_output_success)
    {
        ...
        mixer_config.connection_type = CONNECTION_TYPE_MUSIC_BA;
