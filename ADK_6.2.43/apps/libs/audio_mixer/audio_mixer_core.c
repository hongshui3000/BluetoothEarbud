...
*/
static void mixerCoreConfigureSourceSync(connection_type_t connection_type, uint32 output_sample_rate)
{
...
*/
static void mixerCoreConfigure(connection_type_t connection_type, uint32 output_sample_rate)
{
    Operator mixer_op;
    ...
    if(mixer_op)
    {
        unsigned num_music_channels = mixerCoreGetNumberOfMusicChannels(connection_type);
        unsigned num_downmix_channels = mixerCoreGetNumberOfDownmixChannels(connection_type);
...
{
    const chain_config_t* config = mixerCoreGetChainConfig(connection_type);

    core_data.chain = PanicNull(ChainCreate(config));
    core_data.channel_mode = channel_mode;

    mixerCoreConfigure(connection_type, output_sample_rate);
    mixerCoreConfigureSourceSync(connection_type, output_sample_rate);
