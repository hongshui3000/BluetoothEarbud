...
*/
static void sendLatencyReport (Task app_task, A2dpPluginTaskdata *audio_plugin, bool estimated, uint16 latency, Sink sink)
{
    ...
    message->sink = sink;
    MessageSend(app_task, AUDIO_LATENCY_REPORT, message);
}
...
void CsrA2dpDecoderPluginSetAudioLatency (A2dpPluginTaskdata *audio_plugin, uint16 latency)
{
    if (DECODER != NULL)
    {
        ...
        {
            sendLatencyReport(DECODER->app_task, audio_plugin, FALSE, latency, DECODER->media_sink);
        }
    }
}
...
uint16 getAnalogAudioInputBitResolution(void)
{
    ...
    if(codecData->analogue_in_params->enable_24_bit_resolution)
    {
...
*/
void csrA2dpDecoderUpdateTwsRoutingMode(AUDIO_MUSIC_CHANNEL_MODE_T local_mode, AUDIO_MUSIC_CHANNEL_MODE_T remote_mode)
{
    ...
    if (DECODER != NULL)
    {
        ...
        {   /* Only set routing mode if DSP is *currently* operating as a TWS Master */
            a2dpDecoderDspSendTwsRoutingMode(local_mode, remote_mode);
            DECODER->routing_mode_change_pending = FALSE;
        }
    }
}
...
DESCRIPTION
    ...
    Issues a request to the DSP to change the Stream Relay packet size and
    time interval.
*/
void csrA2dpDecoderSetStreamRelayControl(uint16 amount, uint16 flush_threshold)
{
    audioDecodeDspSendRelayControl(amount, flush_threshold);
}
...
static bool csrA2dpDelaySetLevels(AUDIO_PLUGIN_SET_GROUP_VOLUME_MSG_T* volume_msg)
{
    ...
    if(AudioIsAudioPromptPlaying() || (codecData->sub_is_available == FALSE && codecData->delay_volume_message_sending))
    {
...
static void csrA2dpSetHardwareGainDelayed(audio_output_group_t group, int16 master_gain, uint16 delay_ms)
{
    if(DECODER->task)
    {
        ...
        MessageSendLater((Task)DECODER->task, AUDIO_PLUGIN_DELAY_VOLUME_SET_MSG, message, delay_ms);
    }
    else
    {
...
*/
static void csrA2dpDecoderPluginSetGroupLevels(audio_output_group_t group, int16 master)
{
    ...
    tone_volume = AudioConfigGetToneVolumeToUse();

    /* Get the digital gain settings to apply */
    AudioOutputGainGetDigital(group, master, tone_volume, &gain_info);
    ...
    switch(AudioOutputGainGetType(group))
    {
        case audio_output_gain_hardware:
        {
            if(is_tws_relay_mode() && (group == audio_output_group_main))
            {
                ...
                AudioOutputGainGetDigitalOnly(group, master, tone_volume, &gain_info);
...
void CsrA2dpDecoderPluginSetLevels(AUDIO_PLUGIN_SET_GROUP_VOLUME_MSG_T * volume_msg, bool force_set_volume)
{
    ...
    else
    {
        ...
        if(volume_msg->group == audio_output_group_main || volume_msg->group == audio_output_group_all)
        {
            csrA2dpDecoderPluginSetGroupLevels(audio_output_group_main, volume_msg->main.master);
        ...
        if(volume_msg->group == audio_output_group_aux || volume_msg->group == audio_output_group_all)
        {
            csrA2dpDecoderPluginSetGroupLevels(audio_output_group_aux, volume_msg->aux.master);
        }
    }
...
static void csrA2dpDecoderNotifyAppDspReady(A2dpPluginTaskdata* task)
{
    ...
    MAKE_AUDIO_MESSAGE(AUDIO_DSP_READY_FOR_DATA, message);
    ...
    MessageSend(DECODER->app_task, AUDIO_DSP_READY_FOR_DATA, message);
}
...
void csrA2dpDecoderDspLoadedAndConfigured(A2dpPluginTaskdata* task)
{
    ...
    if (DECODER)
    {
        ...
        /* only release audio busy flag if we don't anticipate ROM patches. 
           By suppressing Audio Busy when ROM patching may occur we prevent
           cases where the DSP can be given VP/tone data before it is ready */
        if(VmalDspInPatchableRom() == FALSE)
        {
            SetAudioBusy( NULL ) ;
        }  
...
void MusicConnectAudio (void)
{
    ...
    DECODER->dsp_resample_rate = (uint16)(AudioOutputGetSampleRate(&local_config->mch_params, 0)/DSP_RESAMPLING_RATE_COEFFICIENT);
...
void csrA2dpDecoderPluginMicMute(AUDIO_MUTE_STATE_T state)
{
    ...
    {
        Source src = AudioPluginGetMicSource(codecData->mic_params->mic_a, AUDIO_CHANNEL_A);
        ...
        {
            src = AudioPluginGetMicSource(codecData->mic_params->mic_b, AUDIO_CHANNEL_B);
...
void CsrA2dpDecoderPluginUpdateTwsChannelMode(void)
{
    csrA2dpDecoderUpdateTwsRoutingMode(AudioConfigGetTwsChannelModeLocal(), AudioConfigGetTwsChannelModeRemote());
}
