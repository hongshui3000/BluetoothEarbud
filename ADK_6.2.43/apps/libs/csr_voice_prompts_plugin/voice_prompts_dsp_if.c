...
void VoicePromptsDspPlay(vp_context_t *context)
{
    if(!VoicePromptsIsCompressed(context->codec_type))
    {
        ...
        else
        {
            ...
            VoicePromptsNoDspPlay(context->codec_type, context->playback_rate, 
...
/*******************************************************************************
DESCRIPTION
    Message handler for long DSP messages.
*/
void VoicePromptsDspLongMessageHandler(vp_context_t *context, Task task, Message message)
{
    ...
    PRINT(("VoicePromptsDspLongMessageHandler: id 0x%04X\n", m->id));
        ...
        unsigned playback_rate, unsigned resample_rate_with_coefficient_applied)
{
    ...
    if (codec_type == voice_prompts_codec_pcm)
    {
        ...
        KalimbaSendMessage(MESSAGE_SET_MUSIC_MANAGER_SAMPLE_RATE, (uint16)resample_rate_with_coefficient_applied, 0, 0, (LOCAL_FILE_PLAYBACK|PCM_PLAYBACK));

        /* Configure prompt playback */
        KalimbaSendMessage(MESSAGE_SET_TONE_RATE_MESSAGE_ID, (uint16)playback_rate, (is_stereo?PROMPT_STEREO:0) | NO_EXTRA_GAIN, 0, 0);
    ...
    else if(codec_type == voice_prompts_codec_tone)
    {
        ...
        KalimbaSendMessage(MESSAGE_SET_MUSIC_MANAGER_SAMPLE_RATE, (uint16)resample_rate_with_coefficient_applied, 0, 0, (PCM_PLAYBACK));

        /* Configure prompt playback */
        KalimbaSendMessage(MESSAGE_SET_TONE_RATE_MESSAGE_ID, (uint16)playback_rate, NO_EXTRA_GAIN, 0, 0);
...
static void setupRunningDspToPlayAudioPrompt(unsigned playback_rate, bool is_stereo)
{
    uint16 flags = (is_stereo ? PROMPT_STEREO : 0) | NO_EXTRA_GAIN;
