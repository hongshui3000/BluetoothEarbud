...
static Source createSpdifSource(spdif_source_t source_type)
{
    ...
    switch(source_type)
    {
        ...
        {
            source = StreamAudioSource(AUDIO_HARDWARE_SPDIF, AUDIO_INSTANCE_0, SPDIF_CHANNEL_A);
            break;
        }
        case spdif_source_right:
        {
            source = StreamAudioSource(AUDIO_HARDWARE_SPDIF, AUDIO_INSTANCE_0, SPDIF_CHANNEL_B);
            break;
        }
        case spdif_source_interleaved:
            source = StreamAudioSource(AUDIO_HARDWARE_SPDIF, AUDIO_INSTANCE_0, SPDIF_CHANNEL_A_B_INTERLEAVED);
            break;
        default:
            Panic();
    }

    PanicZero((int)source);
...
static void connectSourcesToChain(audio_input_context_t *ctx)
{
    ...
    if(interleaved_sink)
    {
        PanicFalse((int)StreamConnect(ctx->left_source, interleaved_sink));
    }
    else
    {
        PanicFalse((int)StreamConnect(ctx->left_source, ChainGetInput(ctx->chain, spdif_input_left)));
        PanicFalse((int)StreamConnect(ctx->right_source, ChainGetInput(ctx->chain, spdif_input_right)));
    }
}
