...
class Variable(DataSym):
    ...
    TODO: This may change (hopefully not by too much) when we know
    for sure how Kalelfreader_lib_wrappers represents variables/structures.
    """
    ...
    def _debug_var_to_str(self):
        ...
        mystr = self.indent + string.replace(mystr, '\n', '\n' + self.indent)
    ...
    def __getitem__(self, key):
        ...
        if self.array_len > 0:
            ...
            # simple int.
            if item.value and \
                    not isinstance(item.value, (int, long)) and \
                    len(item.value) == 1:
