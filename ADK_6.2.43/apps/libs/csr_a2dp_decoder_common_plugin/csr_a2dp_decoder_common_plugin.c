...
const A2dpPluginTaskdata csr_tws_master_sbc_decoder_plugin = {{AudioPluginMusicMessageHandler}, TWS_SBC_DECODER, BITFIELD_CAST(8, 0)};
const A2dpPluginTaskdata csr_tws_master_mp3_decoder_plugin = {{AudioPluginMusicMessageHandler}, TWS_MP3_DECODER, BITFIELD_CAST(8, 0)};
const A2dpPluginTaskdata csr_tws_master_aac_decoder_plugin = {{AudioPluginMusicMessageHandler}, TWS_AAC_DECODER, BITFIELD_CAST(8, 0)};
const A2dpPluginTaskdata csr_tws_master_aptx_decoder_plugin = {{AudioPluginMusicMessageHandler}, TWS_APTX_DECODER, BITFIELD_CAST(8, 0)};
const A2dpPluginTaskdata csr_tws_slave_sbc_decoder_plugin = {{AudioPluginMusicMessageHandler}, TWS_SBC_DECODER, BITFIELD_CAST(8, 0)};
const A2dpPluginTaskdata csr_tws_slave_mp3_decoder_plugin = {{AudioPluginMusicMessageHandler}, TWS_MP3_DECODER, BITFIELD_CAST(8, 0)};
const A2dpPluginTaskdata csr_tws_slave_aac_decoder_plugin = {{AudioPluginMusicMessageHandler}, TWS_AAC_DECODER, BITFIELD_CAST(8, 0)};
const A2dpPluginTaskdata csr_tws_slave_aptx_decoder_plugin = {{AudioPluginMusicMessageHandler}, TWS_APTX_DECODER, BITFIELD_CAST(8, 0)};
...
const A2dpPluginTaskdata csr_usb_decoder_plugin = {{AudioPluginMusicMessageHandler}, SBC_DECODER, BITFIELD_CAST(8, 0)};
...
static void handleAudioMessage ( Task task , MessageId id, Message message )
{
    switch (id)
    {
        ...
        {
            AUDIO_PLUGIN_SET_USER_EQ_PARAMETER_MSG_T* eq_message = (AUDIO_PLUGIN_SET_USER_EQ_PARAMETER_MSG_T*)message ;
            CsrA2dpDecoderSetUserEqParameter(&eq_message->param);
        }
        break;

        case AUDIO_PLUGIN_APPLY_USER_EQ_PARAMETERS_MSG:
        {
            AUDIO_PLUGIN_APPLY_USER_EQ_PARAMETERS_MSG_T* apply_message = (AUDIO_PLUGIN_APPLY_USER_EQ_PARAMETERS_MSG_T*)message;
            CsrA2dpDecoderApplyUserEqParameters(apply_message);
            break;
        }

        case AUDIO_PLUGIN_GET_USER_EQ_PARAMETER_MSG:
        {
            AUDIO_PLUGIN_GET_USER_EQ_PARAMETER_MSG_T* eq_message = (AUDIO_PLUGIN_GET_USER_EQ_PARAMETER_MSG_T*)message ;
            CsrA2dpDecoderGetUserEqParameter(&eq_message->param_id);
        }
        break;

        case AUDIO_PLUGIN_GET_USER_EQ_PARAMETERS_MSG:
        {
            AUDIO_PLUGIN_GET_USER_EQ_PARAMETERS_MSG_T* eq_group_message = (AUDIO_PLUGIN_GET_USER_EQ_PARAMETERS_MSG_T*)message;
            CsrA2dpDecoderGetUserEqGroupParameter(eq_group_message->number_of_params, eq_group_message->param_ids);
            break;
        }

        case AUDIO_PLUGIN_SET_CHANNEL_MODE_MSG:
        {
            CsrA2dpDecoderPluginUpdateTwsChannelMode();
            break;
        }
        ...
        case AUDIO_PLUGIN_DSP_PATCH_PROCESS_END_MSG :
        {
            /* DSP patched and is now ready to process VP and Tones etc */
            SetAudioBusy( NULL ) ;
        }
        break;
