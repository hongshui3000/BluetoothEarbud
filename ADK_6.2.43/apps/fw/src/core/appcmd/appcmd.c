...
/**
 * \defgroup appcmd_buf AppCmd buffer interface
 * \ingroup appcmd
 */
/*@{*/
/**
...
void appcmd_background_handler(void)
{
    ...
    case APPCMD_BUF_MSG_CONSUMED_MSG_IND:
        ...
        break;
    case APPCMD_GET_RSP_BUFFER:
        appcmd_results[0] = (uint32)appcmd_rsp_buf;
        APPCMD_RESPOND(APPCMD_RESPONSE_SUCCESS);
        break;
...
void appcmd_init(void)
{
    ...
    appcmd_rsp_buf = pnew(BUFFER_MSG);
}
...
/**
 * \defgroup AppCmd scheduler interface
 * \ingroup appcmd
 */
/*@{*/
