...
typedef enum
{
    ...
    capability_id_download_cvc_spk_1mic_wb = 0x4003,
    capability_id_anc_tuning           = 0x4082,
...
typedef enum
{
    ...
    rtp_codec_type_sbc,
    rtp_codec_type_atrac,
    rtp_codec_type_mp3,
    rtp_codec_type_aac,
    rtp_codec_type_aptx_hd
} rtp_codec_type_t;

typedef enum
{
    rtp_passthrough = 0,    /* When no RTP header is present. */
    rtp_decode,             /* Decodes the RTP header and calls the TTP module
                               to get the time to play information. */
    rtp_strip,              /* Decodes the RTP header and strips it. */
    rtp_ttp_only            /* Adds time to play information to encoded streams
                               which have no RTP headers*/
} rtp_working_mode_t;
...
/* See A2DP specification for description of SBC codec parameters
   Sample rate unit is Hz.
 */
typedef struct
{
...
#define DEFAULT_OPERATOR_PRIORITY   (operator_priority_medium)
